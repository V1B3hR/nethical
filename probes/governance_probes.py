        ...
        try:
            ...
            return ProbeResult(
                ...
                metrics={
                    ...
                    "compliance_rate": (
                        (len(recent_changes) - insufficient_sigs) / len(recent_changes)
                        if recent_changes
                        else 1.0
                    ),
                },
        ...
        try:
            ...
            for policy in policies:
                ...
                if not self._verify_hash_chain(lineage):
                    broken_chains += 1
                    violations.append(f"Policy {policy_id}: broken hash chain detected")
            ...
            return ProbeResult(
                ...
                metrics={
                    ...
                    "integrity_rate": (
                        (len(policies) - broken_chains) / len(policies)
                        if policies
                        else 1.0
                    ),
                },
...
class TenantIsolationProbe(BaseProbe):
    ...
    def check(self) -> ProbeResult:
        ...
        if violation_rate > 0.01:  # >1% violations (stricter than data min)
            status = ProbeStatus.CRITICAL
            message = (
                f"Tenant isolation breach: {cross_tenant_count} cross-tenant accesses"
            )

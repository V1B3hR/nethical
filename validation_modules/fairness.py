...
class FairnessMetrics:
    ...
    def calculate_demographic_parity_difference(
        self, y_pred: np.ndarray, sensitive_feature: np.ndarray
    ) -> Dict[str, float]:
        """
        ...
        if len(groups) < 2:
            logger.warning(
                "Less than 2 groups found for demographic parity calculation"
            )
    ...
    def calculate_equalized_odds_difference(
        self, y_true: np.ndarray, y_pred: np.ndarray, sensitive_feature: np.ndarray
    ) -> Dict[str, float]:
        """
    ...
    def calculate_all_fairness_metrics(
        ...
        if sensitive_feature is not None:
            try:
                ...
                eod = self.calculate_equalized_odds_difference(
                    y_true, y_pred, sensitive_feature
                )
                metrics["equalized_odds"] = eod

                logger.info(
                    f"Calculated fairness metrics: DPD={dpd['demographic_parity_difference']:.4f}, "

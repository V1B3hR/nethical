...
class DataIntegrityValidator:
    ...
    def validate_schema(
        self, df: pd.DataFrame, schema_config: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        max_null_pct = (
            schema_config.get("max_null_percentage", 0.05) * 100
            if schema_config
            else 5.0
        )
    ...
    def detect_drift_ks_test(
        self, reference: np.ndarray, current: np.ndarray, alpha: float = 0.05
    ) -> Dict[str, Any]:
        """
    ...
    def analyze_drift(
        ...
        psi_drifted = sum(
            1 for v in results["drift_detected"].values() if v.get("psi_drift", False)
        )
        ks_drifted = sum(
            1 for v in results["drift_detected"].values() if v.get("ks_drift", False)
        )
        ...
        results["summary"] = {
            ...
            "psi_drift_percentage": (
                psi_drifted / total_features if total_features > 0 else 0.0
            ),
            "ks_drift_percentage": (
                ks_drifted / total_features if total_features > 0 else 0.0
            ),
        }

        logger.info(
            f"Drift analysis: {psi_drifted}/{total_features} features with PSI drift, "
    ...
    def detect_leakage(
        ...
        if id_columns:
            ...
            results["leakage_percentage"] = (
                len(overlap) / len(test_df) * 100 if len(test_df) > 0 else 0.0
            )
        else:
        ...
        if results["leakage_detected"]:
            logger.warning(
                f"Data leakage detected: {results['leaked_samples']} samples "

...
def test_train_with_drift_tracking():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            "Training cohort ID: test_cohort_a" in combined_output
        ), "Cohort ID not set"

        # Check that drift report was generated
        assert (
            "Generating ethical drift report" in combined_output
...
def test_train_with_promotion_failure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd = [
            ...
            "1",
            "--num-samples",
            "20",  # Very small sample size
            "--seed",
            "123",  # Different seed for different results
            "--enable-drift-tracking",
            "--drift-report-dir",
            str(drift_path),
            "--cohort-id",
...
def test_train_without_drift_tracking():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            "Drift tracking enabled" not in result.stdout
        ), "Drift tracking should not be active"
        assert (
            "Drift Report ID:" not in result.stdout
        ), "Drift report should not be generated"
...
def test_multiple_cohorts_drift():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            len(drift_files) >= 1
        ), f"Expected at least 1 drift report, found {len(drift_files)}"
        ...
        for drift_file in drift_files:
            ...
            reports_summary.append(
                {"file": drift_file.name, "cohorts": cohorts_in_report}
            )

...
class ValidationConfig:
    ...
    def from_environment(cls) -> "ValidationConfig":
        ...
        if os.getenv("NETHICAL_SOAK_DURATION"):
            config.medium_soak_duration_seconds = float(
                os.getenv("NETHICAL_SOAK_DURATION")
            )
        return config
...
class ValidationResults:
    ...
    def get_performance_analysis(self) -> Dict[str, Any]:
        ...
        degradation = (
            ((late_mean - early_mean) / early_mean * 100) if early_mean > 0 else 0
        )
    ...
    def _write_markdown_report(self, filepath: Path):
        ...
        with open(filepath, "w") as f:
            ...
            if memory:
                ...
                f.write(
                    f"- Growth: {memory['growth_mb']:.2f} MB ({memory['growth_percent']:+.2f}%)\n"
                )
                f.write(
                    f"- Leak Detected: {'❌ YES' if memory['leak_detected'] else '✅ NO'}\n\n"
                )

            if performance:
                f.write("## Performance Degradation Analysis\n\n")
                f.write(
                    f"- Early Mean Latency: {performance['early_mean_ms']:.2f} ms\n"
                )
                f.write(f"- Late Mean Latency: {performance['late_mean_ms']:.2f} ms\n")
                f.write(f"- Degradation: {performance['degradation_percent']:+.2f}%\n")
                f.write(
                    f"- Degradation Detected: {'❌ YES' if performance['degradation_detected'] else '✅ NO'}\n\n"
                )
...
class AdvancedLoadTestRunner:
        ...
        while not self._stop_metrics.is_set():
            try:
                metric = SystemMetric(
                    ...
                    active_workers=(
                        self.worker_pool.current_workers if self.worker_pool else 0
                    ),
                    requests_in_flight=(
                        self.worker_pool.active_tasks if self.worker_pool else 0
                    ),
                )
                if self.results:
        ...
        try:
            ...
            for i in range(iterations):
                ...
                if (i + 1) % max(1000, iterations // 10) == 0:
                    ...
                    logger.info(
                        f"  Progress: {i + 1:,}/{iterations:,} submitted, {completed:,} completed"
                    )
    ...
    def run_scaling_test(
        ...
        try:
            ...
            for step in range(ramp_steps):
                ...
                step_futures = submit_requests(
                    ExecutionPhase.RAMP_UP, self.config.ramp_step_seconds
                )
            ...
            sustained_futures = submit_requests(
                ExecutionPhase.SUSTAINED, sustained_duration_seconds
            )
            ...
            for step in range(ramp_steps):
                ...
                step_futures = submit_requests(
                    ExecutionPhase.RAMP_DOWN, self.config.ramp_step_seconds
                )
                all_futures.extend(step_futures)
    ...
    def run_soak_test(
        ...
        if perf:
            logger.info(
                f"  Performance degradation: {perf['degradation_percent']:+.2f}%"
            )
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    "not config.getoption('--run-extended')", reason="Extended tests not enabled"
)

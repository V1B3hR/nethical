...
class LoadTester:
    ...
    def run_synchronous_load_test(
        self, num_requests: int = 100, test_actions: List[str] = None
    ) -> Dict:
        """
        ...
        for i in range(num_requests):
            ...
            try:
                result = self.governance.process_action(
                    agent_id="load_tester", action=action_text
                )
                ...
                if elapsed > 0.5:  # >500ms
                    logger.warning(
                        f"Slow request #{i + 1}: {elapsed * 1000:.2f}ms for action '{action_text}'"
                    )
...
def test_baseline_latency_p95(load_tester):
    ...
    logger.info(
        f"  Throughput: {result['successful_requests'] / total_duration:.2f} req/s"
    )
    ...
    if p95_ms >= 200:
        ...
        logger.error("\nTo reproduce:")
        logger.error(
            "  pytest tests/validation/test_performance_validation.py::test_baseline_latency_p95 -v -s"
        )
...
def test_error_rate_baseline(load_tester):
    ...
    print(f"\nError Rate Test:")
    print(
        f"  Error Rate: {result['error_rate']:.2%} {'✓' if result['error_rate'] < 0.005 else '✗'}"
    )
    ...
    if result["failed_requests"] > 0:
        ...
        for detail in failure_details:
            ...
            error_type = (
                error_msg.split(":")[0]
                if ":" in error_msg
                else error_msg.split()[0] if error_msg.split() else "Unknown"
            )
    ...
    if result["error_rate"] >= 0.005:
        ...
        logger.error("\nTo reproduce:")
        logger.error(
            "  pytest tests/validation/test_performance_validation.py::test_error_rate_baseline -v -s"
        )
...
def test_sustained_load_performance(load_tester):
    ...
    for batch in range(batches):
        ...
        print(
            f"  Batch {batch + 1}: P95={result['latency_metrics']['p95'] * 1000:.2f}ms, "
    ...
    ), f"Sustained load error rate {overall_error_rate:.2%} exceeds SLO"
...
def test_soak_test_stability(load_tester):
    ...
    for batch in range(duration_batches):
        ...
        p95_ms = result["latency_metrics"]["p95"] * 1000
        print(
            f"  Batch {batch + 1}: P95={p95_ms:.2f}ms, Errors={result['failed_requests']}"
        )
    ...
    assert (
        degradation < 0.5
    ), f"Performance degraded by {degradation:.1%} during soak test"
...
def test_generate_performance_report(load_tester, tmp_path):
    ...
    for scenario_name, num_requests in scenarios.items():
        ...
        latency_metrics_ms = {k: v * 1000 for k, v in result["latency_metrics"].items()}
    ...
    # Overall compliance
    all_compliant = all(
        test["slo_compliance"]["p95_under_200ms"]
        and test["slo_compliance"]["error_rate_under_0.5pct"]

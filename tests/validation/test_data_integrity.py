                ...
                if "merkle_root" in prev_entry:
                    ...
                    if "previous_root" in entry:
                        if entry["previous_root"] != prev_entry["merkle_root"]:
                            issues.append(
                                f"Block {i}: Chain break - previous_root mismatch"
                            )
                            continue
        ...
        for i, action in enumerate(actions):
            try:
                ...
                result = governance.process_action(
                    agent_id="test_agent", action=action_content
                )
                replayed += 1
...
def test_merkle_chain_continuity(integrity_validator, merkle_anchor):
    ...
    logger.info(
        f"  Verification Rate: {result['verification_rate']:.2%} (threshold: 100%)"
    )
    ...
    print(
        f"  Verification Rate: {result['verification_rate']:.2%} {'✓' if result['verification_rate'] == 1.0 else '✗'}"
    )
    print(
        f"  Chain Valid: {result['chain_valid']} {'✓' if result['chain_valid'] else '✗'}"
    )
    ...
    if not result["chain_valid"]:
        ...
        logger.error("\nTo reproduce:")
        logger.error(
            "  pytest tests/validation/test_data_integrity.py::test_merkle_chain_continuity -v -s"
        )
...
def test_merkle_chain_break_detection(integrity_validator):
    ...
    audit_trail = [
        {"block_id": 0, "merkle_root": hashlib.sha256(b"block_0").hexdigest()},
        {
            "block_id": 1,
...
def test_audit_replay_verification(integrity_validator, governance):
    ...
    print(
        f"  Replay Rate: {result['replay_rate']:.2%} {'✓' if result['replay_rate'] >= 0.95 else '✗'}"
    )
    print(
        f"  Replay Successful: {result['replay_successful']} {'✓' if result['replay_successful'] else '✗'}"
    )
...
def test_integrity_with_merkle_anchor(merkle_anchor):
    ...
    test_data = [
        {"action_id": f"test_{i}", "data": f"test data {i}"} for i in range(10)
    ]
...
def test_generate_integrity_report(
    integrity_validator, governance, merkle_anchor, tmp_path
):

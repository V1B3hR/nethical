...
class ActionDistributionSimulator:
    ...
    @staticmethod
    def generate_shifted_distribution(
        size: int = 1000, seed: int = 44, shift: float = 0.15
    ) -> np.ndarray:
...
def test_psi_no_drift(detector, simulator):
    ...
    logger.info(
        f"Reference distribution: mean={reference.mean():.4f}, std={reference.std():.4f}, size={len(reference)}"
    )
    logger.info(
        f"Current distribution: mean={current.mean():.4f}, std={current.std():.4f}, size={len(current)}"
    )
    ...
    logger.info(
        f"  Overall Drift: {'DETECTED' if result['drift_detected'] else 'NOT DETECTED'}"
    )

    print(f"\nNo Drift Test:")
    print(f"  PSI: {result['psi']:.4f} {'✓' if result['psi'] < 0.2 else '✗'}")
    print(
        f"  KS p-value: {result['ks_p_value']:.4f} {'✓' if result['ks_p_value'] > 0.05 else '✗'}"
    )
    print(
        f"  Drift Detected: {result['drift_detected']} {'✓ (correct)' if not result['drift_detected'] else '✗ (false positive)'}"
    )
    ...
    if result["drift_detected"]:
        ...
        logger.error(
            f"4. Current PSI={result['psi']:.4f}, KS p-value={result['ks_p_value']:.4f}"
        )
...
def test_psi_significant_drift(detector, simulator):
    ...
    logger.info(
        f"Reference distribution: mean={reference.mean():.4f}, std={reference.std():.4f}"
    )
    logger.info(
        f"Current distribution: mean={current.mean():.4f}, std={current.std():.4f}"
    )
    ...
    print(f"  KS p-value: {result['ks_p_value']:.4f}")
    print(
        f"  Drift Detected: {result['drift_detected']} {'✓ (correct)' if result['drift_detected'] else '✗ (missed)'}"
    )
    ...
    if not result["drift_detected"]:
        ...
        logger.error(
            f"4. Distribution means: ref={reference.mean():.4f}, cur={current.mean():.4f}"
        )
...
def test_weekly_drift_monitoring(detector, simulator):
    ...
    for result in weekly_results:
        print(
            f"  Day {result['day']}: PSI={result['psi']:.4f}, Drift={result['drift_detected']}"
        )
    ...
    assert weekly_results[5][
        "drift_detected"
    ], f"Day 6 should detect drift (PSI={weekly_results[5]['psi']:.4f}, shift=0.05)"
    assert weekly_results[6][
        "drift_detected"
    ], f"Day 7 should detect drift (PSI={weekly_results[6]['psi']:.4f}, shift=0.06)"

...
class AdvancedTestRunner:
    ...
    def run_test_class(self, test_class: str) -> TestClassResult:
        ...
        try:
            ...
            json_report_file = (
                self.individual_dir / f"{test_class.lower()}_results.json"
            )
            ...
            errors = sum(
                1 for t in test_results if t.status in ["ERROR", "INTERRUPTED"]
            )
            ...
            print(
                f"   Total: {len(test_results)}, Passed: {passed}, Failed: {failed}, Skipped: {skipped}, Errors: {errors}"
            )
    ...
    def save_class_results(
        self, class_result: TestClassResult, stdout: str, stderr: str
    ):
    ...
    def generate_reports(self, overall_result: OverallResult):
        ...
        with open(text_file, "w") as f:
            ...
            success_rate = (
                overall_result.passed / max(overall_result.total_tests, 1)
            ) * 100
                ...
                if class_result.failed > 0:
                    ...
                    for test in class_result.tests:
                        if test.status == "FAILED":
                            f.write(
                                f"    - {test.test_name}: {test.error_message or 'No details'}\n"
                            )
...
"""
        ...
        for class_result in overall_result.test_classes:
            success_rate = (
                class_result.passed / max(class_result.total_tests, 1)
            ) * 100
            html_content += f"""

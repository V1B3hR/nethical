    ...
    def test_default_deny_policy_exists(self):
        ...
        default_deny = next(
            (
                doc
                for doc in docs
                if doc.get("metadata", {}).get("name") == "default-deny-all"
            ),
            None,
        )
...
class TestServiceMeshConfig:
    ...
    def test_mtls_strict_mode(self):
        ...
        assert peer_auth["spec"]["mtls"]["mode"] == "STRICT", "mTLS mode is not STRICT"
    ...
    def test_authorization_policies(self):
        ...
        auth_policies = [
            doc for doc in docs if doc and doc.get("kind") == "AuthorizationPolicy"
        ]

        assert len(auth_policies) >= 3, "Insufficient authorization policies configured"
...
class TestExternalSecrets:
    ...
    def test_secret_stores_defined(self):
        ...
        secret_stores = [doc for doc in docs if doc.get("kind") == "SecretStore"]
    ...
    def test_jwt_keys_external_secret(self):
        ...
        assert (
            jwt_secret["spec"]["refreshInterval"] == "1h"
...
class TestRuntimeSecurity:
    ...
    def test_statefulset_security_context(self):
        ...
        assert (
            pod_security["runAsNonRoot"] is True
    ...
    def test_container_security_context(self):
        ...
        assert "ALL" in container_security.get("capabilities", {}).get(
            "drop", []
...
class TestWAFConfiguration:
    ...
    def test_modsecurity_rules(self):
        ...
        assert (
            "ignore" in rules_content.lower()
            and "previous" in rules_content.lower()
            and "instruction" in rules_content.lower()
        ), "Prompt injection rule not found"
        assert (
            "jailbreak" in rules_content.lower()
        ), "Jailbreak detection rule not found"

        # Check for resource limits
        assert (
            "SecRequestBodyLimit" in rules_content
    ...
    def test_ingress_waf_annotations(self):
        ...
        ingress = next((doc for doc in docs if doc.get("kind") == "Ingress"), None)

        assert ingress is not None, "Ingress with WAF not found"

        annotations = ingress["metadata"]["annotations"]
        assert (
            annotations.get("nginx.ingress.kubernetes.io/enable-modsecurity") == "true"
        ), "ModSecurity not enabled on ingress"
        assert (
            annotations.get("nginx.ingress.kubernetes.io/proxy-body-size") == "1m"
...
class TestSecurityDocumentation:
    ...
    def test_security_hardening_guide_completeness(self):
        ...
        for section in required_sections:
            assert (
                section in content
            ), f"Section '{section}' not found in security guide"

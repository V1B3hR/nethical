...
def sample_judgment_data_safe():
    ...
    return {
        ...
        "decision": "ALLOW",
        "violations": [],
        "phase3": {"risk_score": 0.1, "risk_tier": "low"},
        "pii_detection": {"matches_count": 0, "pii_risk_score": 0.0},
        "phase4": {"ethical_tags": {}},
        "quota_enforcement": {"backpressure_level": 0.2},
    }
...
def sample_judgment_data_violation():
    ...
    return {
        ...
        "phase3": {"risk_score": 0.85, "risk_tier": "critical"},
        "pii_detection": {"matches_count": 3, "pii_risk_score": 0.9},
        "phase4": {
            "ethical_tags": {
                "primary_dimension": "privacy",
                "dimensions": {"privacy": 0.95, "security": 0.80},
            }
        },
        "quota_enforcement": {"backpressure_level": 0.7},
    }
...
def test_explain_safe_decision(explainer, sample_judgment_data_safe):
    ...
    explanation = explainer.explain_decision_with_features(
        decision="ALLOW", judgment_data=sample_judgment_data_safe, confidence=0.95
    )
...
def test_explain_violation_decision(explainer, sample_judgment_data_violation):
    ...
    explanation = explainer.explain_decision_with_features(
        decision="BLOCK", judgment_data=sample_judgment_data_violation, confidence=0.90
    )
    ...
    violation_feature = next(
        (
            ...
            if f.feature_name == "violation_count"
        ),
        None,
    )
...
def test_feature_importance_ordering(explainer, sample_judgment_data_violation):
    ...
    explanation = explainer.explain_decision_with_features(
        decision="BLOCK", judgment_data=sample_judgment_data_violation
    )
...
def test_generate_counterfactual_block_to_allow(
    explainer, sample_judgment_data_violation
):
    ...
    violation_change = next(
        (
            c
            for c in counterfactual.required_changes
            if c["feature"] == "violation_count"
        ),
        None,
    )

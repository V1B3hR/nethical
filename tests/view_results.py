...
def show_summary():
    ...
    if latest_json and latest_json.exists():
        ...
        for test_class in data['test_classes']:
            success_rate = (test_class['passed'] / max(test_class['total_tests'], 1)) * 100
            status_icon = "✅" if success_rate == 100 else "⚠️" if success_rate >= 50 else "❌"
            print(f"{status_icon} {test_class['class_name']}: {test_class['passed']}/{test_class['total_tests']} ({success_rate:.1f}%)")
...
def list_available_reports():
    ...
    # HTML Reports
    html_reports = sorted(results_dir.glob("reports/test_report_*.html"), key=os.path.getctime, reverse=True)
    if html_reports:
        print("\n🌐 HTML Reports:")
    ...
    # Text Summaries
    text_summaries = sorted(results_dir.glob("summary/test_summary_*.txt"), key=os.path.getctime, reverse=True)
    if text_summaries:
        print("\n📄 Text Summaries:")
    ...
    # JSON Reports
    json_reports = sorted(results_dir.glob("reports/full_report_*.json"), key=os.path.getctime, reverse=True)
    if json_reports:
        print("\n📊 JSON Reports:")
    ...
    # Individual Results
    individual_results = sorted(results_dir.glob("individual/*_detailed.json"), key=os.path.getctime, reverse=True)
    ...
    if not json_file.exists():
        ...
        if matches:
            ...
            for file in results_dir.glob("*_detailed.json"):
                class_name_from_file = file.stem.replace("_detailed", "").replace("test", "Test")
...
def main():
    ...
    parser.add_argument("--html", action="store_true", help="Open HTML report in browser")
    parser.add_argument("--list", "-l", action="store_true", help="List available reports")
    parser.add_argument("--class", "-c", dest="class_name", help="Show results for specific test class")

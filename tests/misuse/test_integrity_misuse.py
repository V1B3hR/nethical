...
class TestAuditLogBackdating:
    ...
    def test_reject_backdated_entry(self, mock_audit_log):
        ...
        with pytest.raises(ValueError, match="Backdated entry not allowed.*P-NO-BACKDATE"):
...
class TestLogInjection:
    ...
    def test_ansi_escape_injection_blocked(self):
        ...
        sanitized = malicious_input.replace('\x1b', '').replace('[', '').replace('m', '')

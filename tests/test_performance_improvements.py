...
class TestDifferentialPrivacyPerformance:
    ...
    def test_vectorized_noise_addition_performance(self):
        ...
        dp = DifferentialPrivacy(epsilon=1.0, delta=1e-5, mechanism=PrivacyMechanism.GAUSSIAN)
        ...
        assert elapsed < 0.1, f"Vectorized noise addition took {elapsed}s, expected < 0.1s"
    ...
    def test_vectorized_noise_budget_accounting(self):
        ...
        dp = DifferentialPrivacy(epsilon=3.0, delta=1e-5)  # Increased budget for multiple operations
...
class TestHealthcarePipelinePerformance:
    ...
    def test_no_unnecessary_deepcopy(self):
        ...
        filepath = Path(__file__).parent.parent / 'nethical' / 'integrations' / 'healthcare_pipeline.py'

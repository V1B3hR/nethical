...
class CompressionMetrics:
    ...
    def add_tier_metric(
        self, tier: str, original_size: int, compressed_size: int, compression_type: str
    ):
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get all statistics"""
        stats = {"tiers": {}, "totals": {"original": 0, "compressed": 0}}
        ...
        if stats["totals"]["compressed"] > 0:
            stats["totals"]["ratio"] = (
                stats["totals"]["original"] / stats["totals"]["compressed"]
            )
        else:
    ...
    def save_report(self, output_dir: Path) -> str:
        ...
        with open(report_file, "w") as f:
            ...
            f.write(
                f"- Total Compressed: {stats['totals']['compressed']/1024:.1f} KB\n"
            )
...
def test_tier_compression_ratios(compression_metrics, sample_data):
    ...
    assert (
        ratio >= 0.95 and ratio <= 1.05
    ), f"Hot tier ratio should be ~1:1, got {ratio:.2f}:1"

    # Warm tier - LZ4
    print("\n2. Warm Tier (LZ4):")
    compressed_lz4 = compress_lz4_simulated(sample_data)
    compression_metrics.add_tier_metric(
        "warm", original_size, len(compressed_lz4), "lz4"
    )
    ratio = compression_metrics.get_tier_ratio("warm")
    print(
        f"   Compressed: {len(compressed_lz4)} bytes ({len(compressed_lz4)/1024:.1f} KB)"
    )
    print(f"   Ratio: {ratio:.2f}:1")
    assert (
        ratio >= 1.5 and ratio <= 3.0
    ), f"Warm tier ratio should be ~2:1, got {ratio:.2f}:1"

    # Cool tier - ZSTD
    print("\n3. Cool Tier (ZSTD):")
    compressed_zstd = compress_zstd_simulated(sample_data, level=3)
    compression_metrics.add_tier_metric(
        "cool", original_size, len(compressed_zstd), "zstd"
    )
    ratio = compression_metrics.get_tier_ratio("cool")
    print(
        f"   Compressed: {len(compressed_zstd)} bytes ({len(compressed_zstd)/1024:.1f} KB)"
    )
    print(f"   Ratio: {ratio:.2f}:1")
    assert (
        ratio >= 4.0 and ratio <= 7.0
    ), f"Cool tier ratio should be ~5:1, got {ratio:.2f}:1"

    # Cold tier - ZSTD max
    print("\n4. Cold Tier (ZSTD max):")
    compressed_zstd_max = compress_zstd_simulated(sample_data, level=19)
    compression_metrics.add_tier_metric(
        "cold", original_size, len(compressed_zstd_max), "zstd-max"
    )
    ratio = compression_metrics.get_tier_ratio("cold")
    print(
        f"   Compressed: {len(compressed_zstd_max)} bytes ({len(compressed_zstd_max)/1024:.1f} KB)"
    )
    print(f"   Ratio: {ratio:.2f}:1")
    assert (
        ratio >= 8.0 and ratio <= 12.0
    ), f"Cold tier ratio should be ~10:1, got {ratio:.2f}:1"

    # Archive tier - LZMA
    print("\n5. Archive Tier (LZMA):")
    compressed_lzma = compress_lzma_simulated(sample_data)
    compression_metrics.add_tier_metric(
        "archive", original_size, len(compressed_lzma), "lzma"
    )
    ratio = compression_metrics.get_tier_ratio("archive")
    print(
        f"   Compressed: {len(compressed_lzma)} bytes ({len(compressed_lzma)/1024:.1f} KB)"
    )
    print(f"   Ratio: {ratio:.2f}:1")
    assert (
        ratio >= 12.0 and ratio <= 18.0
    ), f"Archive tier ratio should be ~15:1, got {ratio:.2f}:1"
...
def test_aggregate_compression_ratio(compression_metrics, sample_data, output_dir):
    ...
    assert (
        passes
    ), f"Aggregate compression ratio too low: {aggregate_ratio:.2f}:1 < {target_ratio}:1"
...
def test_compression_ratio_with_different_data_types(compression_metrics):
    ...
    data_types = {"json": json_data, "metrics": metrics_data, "binary": binary_data}

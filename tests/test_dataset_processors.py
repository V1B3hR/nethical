...
def test_cyber_security_processor():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            rec1["features"]["severity_max"] > 0.5
        ), "High severity should map to >0.5"
...
def test_generic_processor():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            records[0]["features"]["severity_max"] >= 0.8
        ), "Critical severity should be high"
...
def test_feature_extraction():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ), f"Missing features: {expected_keys - set(features.keys())}"

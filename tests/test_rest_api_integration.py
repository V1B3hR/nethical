...
class TestEvaluateEndpoint:
    ...
    def test_evaluate_endpoint_exists(self, test_client):
        ...
        response = test_client.post(
            "/evaluate", json={"action": "test action", "agent_id": "test"}
        )
        assert response.status_code == 200

    def test_evaluate_minimal_request(self, test_client):
        """Test evaluate with minimal required fields."""
        response = test_client.post("/evaluate", json={"action": "print('hello')"})
    ...
    def test_evaluate_missing_action(self, test_client):
        ...
        response = test_client.post("/evaluate", json={"agent_id": "test"})
    ...
    def test_evaluate_empty_action(self, test_client):
        ...
        response = test_client.post("/evaluate", json={"action": ""})
    ...
    def test_evaluate_decision_types(self, test_client):
        ...
        response = test_client.post("/evaluate", json={"action": "test action"})
    ...
    def test_evaluate_response_structure(self, test_client):
        ...
        # Optional fields
        assert (
            "risk_score" in data or "risk_score" not in data
...
class TestEvaluateWithDifferentActions:
    ...
    def test_database_command(self, test_client):
        ...
        response = test_client.post(
            "/evaluate",
            json={"action": "SELECT * FROM users", "action_type": "database_query"},
        )
    ...
    def test_file_operation(self, test_client):
        ...
        response = test_client.post(
            "/evaluate",
            json={"action": "read configuration file", "action_type": "file_operation"},
        )
...
class TestPIIDetection:
    ...
    def test_action_with_email(self, test_client):
        ...
        response = test_client.post(
            "/evaluate", json={"action": "Send email to user@example.com"}
        )
    ...
    def test_action_with_multiple_pii(self, test_client):
        ...
        response = test_client.post(
            "/evaluate",
            json={"action": "Contact: email=john@example.com, phone=555-1234"},
        )
...
class TestDefaultValues:
    ...
    def test_default_agent_id(self, test_client):
        ...
        response = test_client.post("/evaluate", json={"action": "test"})
...
class TestErrorHandling:
    ...
    def test_invalid_json(self, test_client):
        ...
        response = test_client.post(
            "/evaluate", data="not json", headers={"Content-Type": "application/json"}
        )
    ...
    def test_action_too_long(self, test_client):
        ...
        response = test_client.post("/evaluate", json={"action": long_action})
    ...
    def test_invalid_field_type(self, test_client):
        ...
        response = test_client.post(
            "/evaluate", json={"action": 12345}  # Should be string
        )
...
class TestMultipleRequests:
    ...
    def test_concurrent_requests(self, test_client):
        ...
        for action in actions:
            response = test_client.post(
                "/evaluate", json={"action": action, "agent_id": f"test-{action}"}
            )

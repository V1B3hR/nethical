...
class TestQuarantineManager:
    ...
    def test_quarantine_cohort(self):
        ...
        record = manager.quarantine_cohort(
            cohort="test_cohort", reason=QuarantineReason.ANOMALY_DETECTED
        )
    ...
    def test_quarantine_activation_speed(self):
        ...
        start = time.time()
        record = manager.quarantine_cohort(
            cohort="speed_test", reason=QuarantineReason.COORDINATED_ATTACK
        )
...
class TestEthicalTaxonomy:
    ...
    def test_create_tagging(self):
        ...
        tagging = taxonomy.create_tagging(
            violation_type="emotional_manipulation", context={"automated": True}
        )
...
class TestPhase4Integration:
    ...
    def test_integration_initialization(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            gov = Phase4IntegratedGovernance(
                storage_dir=tmpdir, taxonomy_path="ethics_taxonomy.json"
            )
    ...
    def test_quarantine_integration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = gov.process_action(
                agent_id="agent_1", action="test", cohort="blocked_cohort"
            )
    ...
    def test_merkle_verification(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(5):
                gov.process_action(
                    agent_id=f"agent_{i}", action=f"action_{i}", cohort="test"
                )

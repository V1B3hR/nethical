...
class TestActionReplayer:
    ...
    def populated_db(self, temp_db):
        ...
        persistence = PersistenceManager(
            db_path=os.path.join(temp_db, "action_streams.db"), retention_days=365
        )
    ...
    def test_set_time_range_invalid(self, replayer):
        ...
        with pytest.raises(ValueError):
            replayer.set_time_range(
                "2024-01-15T12:00:00Z", "2024-01-15T10:00:00Z"  # End before start
            )
    ...
    def test_get_actions_with_time_range(self, populated_db):
        ...
        replayer.set_time_range("2024-01-15T10:00:00", "2024-01-15T10:30:00")
    ...
    def test_replay_with_policy_basic(self, populated_db):
        ...
        results = replayer.replay_with_policy(new_policy="strict_policy.yaml", limit=10)
    ...
    def test_replay_with_policy_for_specific_agents(self, populated_db):
        ...
        results = replayer.replay_with_policy(
            new_policy="test_policy.yaml", agent_ids=["agent_0", "agent_1"]
        )
    ...
    def test_compare_outcomes_basic(self, populated_db):
        ...
        comparison = replayer.compare_outcomes(
            baseline_policy="current", candidate_policy="strict_policy.yaml", limit=50
        )
    ...
    def test_compare_outcomes_decision_breakdown(self, populated_db):
        ...
        comparison = replayer.compare_outcomes(
            baseline_policy="permissive.yaml", candidate_policy="strict.yaml", limit=30
        )
    ...
    def test_compare_outcomes_restrictiveness_tracking(self, populated_db):
        ...
        comparison = replayer.compare_outcomes(
            baseline_policy="permissive.yaml", candidate_policy="strict.yaml", limit=20
        )
        ...
        assert (
            comparison.more_restrictive + comparison.less_restrictive
    ...
    def test_policy_comparison_serialization(self, populated_db):
        ...
        comparison = replayer.compare_outcomes(
            baseline_policy="current", candidate_policy="strict.yaml", limit=10
        )
...
class TestActionReplayPerformance:
    ...
    def large_db(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            persistence = PersistenceManager(
                db_path=os.path.join(tmpdir, "action_streams.db"), retention_days=365
            )
    ...
    def test_comparison_performance(self, large_db):
        ...
        start = time.perf_counter()
        comparison = replayer.compare_outcomes(
            baseline_policy="permissive.yaml", candidate_policy="strict.yaml", limit=500
        )

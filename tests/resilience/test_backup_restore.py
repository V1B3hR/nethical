...
class BackupRestoreMetrics:
    ...
    def save_artifacts(self, output_dir: Path) -> Dict[str, str]:
        ...
        with open(md_file, "w") as f:
            ...
            f.write(
                f"- Total RTO: {stats['total_time_seconds']:.2f}s ({stats['total_time_seconds']/60:.2f} min)\n"
            )
            ...
            f.write(
                f"- Target RTO: {target_rto_minutes} minutes ({target_rto_seconds}s)\n"
            )
            f.write(
                f"- Actual RTO: {stats['total_time_seconds']/60:.2f} minutes ({stats['total_time_seconds']:.2f}s)\n"
            )
...
class BackupRestoreEngine:
    ...
    async def create_backup(
        self, data_dir: Path, backup_dir: Path
    ) -> Tuple[bool, float]:
        """
        ...
        try:
            if self.simulation_mode:
                ...
                self.metrics.record_event(
                    "backup_completed", {"status": "success", "duration": elapsed}
                )
                return True, elapsed
            else:
                # Actually copy data
            ...
            elapsed = time.time() - start_time
            self.metrics.record_event(
                "backup_failed", {"error": str(e), "duration": elapsed}
            )
            return False, elapsed

    async def restore_backup(
        self, backup_dir: Path, restore_dir: Path
    ) -> Tuple[bool, float]:
        """
        Restore from backup
        ...
        try:
            if self.simulation_mode:
                ...
                self.metrics.record_event(
                    "restore_completed", {"status": "success", "duration": elapsed}
                )
                return True, elapsed
            else:
            ...
            elapsed = time.time() - start_time
            self.metrics.record_event(
                "restore_failed", {"error": str(e), "duration": elapsed}
            )
            return False, elapsed
    ...
    def validate_restore(self, original_dir: Path, restore_dir: Path) -> bool:
        ...
        self.metrics.record_validation(
            "restore_dir_exists", True, "Restore directory exists"
        )
        ...
        if not metadata_file.exists():
            self.metrics.record_validation(
                "metadata_exists", False, "Metadata file not found"
            )
            return False

        self.metrics.record_validation("metadata_exists", True, "Metadata file exists")
        ...
        try:
            ...
            if "timestamp" not in metadata:
                self.metrics.record_validation(
                    "metadata_valid", False, "Metadata missing timestamp"
                )
                return False

            self.metrics.record_validation(
                "metadata_valid", True, f"Metadata valid: {metadata['timestamp']}"
            )
        except Exception as e:
            self.metrics.record_validation(
                "metadata_valid", False, f"Metadata parse error: {e}"
            )
            return False
...
async def test_backup_restore_dry_run(backup_engine, test_dirs, output_dir):
    ...
    backup_success, backup_time = await backup_engine.create_backup(
        data_dir, backup_dir
    )
    ...
    restore_success, restore_time = await backup_engine.restore_backup(
...
async def test_incremental_backup_restore(backup_engine, test_dirs, output_dir):
    ...
    backup_success, backup_time2 = await backup_engine.create_backup(
        data_dir, backup_dir
    )

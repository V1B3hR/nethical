...
class ChaosTestMetrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recovery_times': {
                'mean': sum(r['recovery_time_seconds'] for r in self.recovery_times) / len(self.recovery_times) if self.recovery_times else 0,
                'max': max(r['recovery_time_seconds'] for r in self.recovery_times) if self.recovery_times else 0,
                'min': min(r['recovery_time_seconds'] for r in self.recovery_times) if self.recovery_times else 0,
            },
            'events': self.events,
...
class ChaosEngine:
    ...
    async def pod_kill(self, namespace: str, pod_name: str) -> bool:
        ...
        """
        self.metrics.record_event('pod_kill_initiated', {
            'namespace': namespace,
            'pod': pod_name,
        ...
        if self.simulation_mode:
            ...
            self.metrics.record_event('pod_kill_completed', {
                'namespace': namespace,
                'pod': pod_name,
                'status': 'simulated'
            })
            ...
            try:
                ...
                self.metrics.record_event('pod_kill_error', {
                    'namespace': namespace,
                    'pod': pod_name,
                    'error': str(e)
                })
    ...
    async def check_pod_ready(self, namespace: str, label_selector: str, timeout: int = 60) -> tuple[bool, float]:
        ...
        if self.simulation_mode:
            ...
            self.metrics.record_event('pod_ready_check', {
                'namespace': namespace,
            ...
            while time.time() < end_time:
                try:
                    ...
                    self.metrics.record_event('pod_ready_check_error', {
                        'namespace': namespace,
                        'label_selector': label_selector,
                        'error': str(e)
                    })
    ...
    async def simulate_region_failover(self) -> tuple[bool, float]:
        ...
        self.metrics.record_event('region_failover_initiated', {
            'simulation': self.simulation_mode
        })
        ...
        self.metrics.record_event('region_failover_completed', {
            'status': 'success',
...
async def test_pod_kill_recovery(chaos_engine, output_dir):
    ...
    assert recovery_pass, f"Recovery too slow: {recovery_time:.2f}s > {target_recovery_time}s"
...
async def test_region_failover(chaos_engine, output_dir):
    ...
    assert failover_pass, f"Failover too slow: {failover_time:.2f}s > {target_failover_time}s"
...
async def test_quorum_recovery(chaos_engine, output_dir):
    ...
    print(f"Recovery time ≤ {target_quorum_time}s: {'✅ PASS' if recovery_pass else '❌ FAIL'}")
    
    assert quorum_achieved, f"Quorum not achieved: only {pods_ready}/3 pods ready"
    assert recovery_pass, f"Quorum recovery too slow: {recovery_time:.2f}s > {target_quorum_time}s"

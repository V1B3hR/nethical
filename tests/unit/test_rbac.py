...
class TestRBACManager:
    ...
    def test_role_hierarchy(self):
        ...
        assert (
            self.rbac.ROLE_HIERARCHY[Role.ADMIN]
            > self.rbac.ROLE_HIERARCHY[Role.OPERATOR]
        )
        assert (
            self.rbac.ROLE_HIERARCHY[Role.OPERATOR]
            > self.rbac.ROLE_HIERARCHY[Role.AUDITOR]
        )
        assert (
            self.rbac.ROLE_HIERARCHY[Role.AUDITOR]
            > self.rbac.ROLE_HIERARCHY[Role.VIEWER]
        )
    ...
    def test_access_history(self):
        ...
        self.rbac.check_permission(
            "user1", Permission.READ_METRICS, raise_on_deny=False
        )
    ...
    def test_no_role_assigned(self):
        ...
        decision = self.rbac.check_role(
            "unknown_user", Role.VIEWER, raise_on_deny=False
        )
        assert not decision.allowed
...
class TestRBACIntegration:
    ...
    def test_complete_workflow(self):
        ...
        self.rbac.check_permission(
            "bob", Permission.EXECUTE_ACTIONS, raise_on_deny=False
        )
        history = self.rbac.get_access_history()

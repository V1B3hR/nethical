...
class TestForensicAnalyzer:
    ...
    def test_detect_suspicious_patterns(self):
        ...
        assert any(
            "failed authentication" in p.lower()
...
class TestChainOfCustodyManager:
    ...
    def test_transfer_custody(self):
        ...
        manager.create_evidence(
            "ev-456", "Audit logs", "analyst1@example.com", "server1"
        )
    ...
    def test_access_evidence(self):
        ...
        success = manager.access_evidence(
            "ev-789", "auditor@example.com", "Compliance review"
        )
...
class TestEnhancedAuditLogger:
    ...
    def test_finalize_block(self):
        ...
        for i in range(5):
            logger.log_event(
                AuditEventType.SYSTEM_EVENT, f"user{i}", "test", "test", "success"
            )
    ...
    def test_search_logs(self):
        ...
        logger.log_event(
            AuditEventType.AUTHENTICATION, "user1", "login", "system", "success"
        )
        logger.log_event(AuditEventType.DATA_ACCESS, "user1", "read", "data", "success")
        logger.log_event(
            AuditEventType.AUTHENTICATION, "user2", "login", "system", "success"
        )
    ...
    def test_generate_forensic_report(self):
        ...
        for i in range(10):
            logger.log_event(
                AuditEventType.DATA_ACCESS, "user123", "read", f"resource{i}", "success"
            )

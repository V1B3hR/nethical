...
pytestmark = pytest.mark.skipif(
    not CACHE_AVAILABLE,
    reason="Semantic cache not available - check dependencies"
)
...
class TestCacheSingleFlight:
    ...
    async def test_single_flight_prevents_duplicate_computation(self):
        ...
        tasks = [
            cache.get_or_compute(intent, action, expensive_compute)
            for _ in range(10)
        ]
...
class TestCacheConfigParams:
    ...
    async def test_cache_key_sorted_config(self):
        ...
        key1 = cache._compute_key(
            "test", "test",
            config_params={"a": 1, "b": 2}
        )
        key2 = cache._compute_key(
            "test", "test",
            config_params={"b": 2, "a": 1}
        )

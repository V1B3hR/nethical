...
def test_train_all_model_types():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for model_type in expected_models:
            ...
            assert (
                training_pattern in combined_output
        ...
        ), "Wrong number of models in summary"
        ...
        assert (
            len(model_files) >= 5
        ), f"Expected at least 5 model files, found {len(model_files)}"
        assert (
            len(metrics_files) >= 5
        ), f"Expected at least 5 metrics files, found {len(metrics_files)}"
...
def test_train_all_with_audit_and_governance():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"  ✓ Total data violations: {summary['governance'].get('data_violations', 0)}"
        )
        print(
            f"  ✓ Total prediction violations: {summary['governance'].get('prediction_violations', 0)}"
        )
...
def test_train_all_model_types_command_from_problem_statement():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            "Accuracy >= 0.85" in combined_output
        ), "Accuracy threshold not applied correctly"
        ...
        for model_type in expected_models:
            ...
            if (
                f"{model_type}: ✓ SUCCESS" in combined_output
                or f"{model_type}: ✓ PROMOTED" in combined_output
            ):
                successful_models.append(model_type)

        assert (
            len(successful_models) == 5

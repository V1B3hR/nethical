...
class TestMLflowIntegration:
    ...
    def test_complete_workflow(self):
        ...
        # Log parameters
        integration.log_parameters(
            run_id, {"model": "random_forest", "n_estimators": 100}
        )

        # Log metrics
        integration.log_metrics(run_id, {"train_accuracy": 0.98, "val_accuracy": 0.95})
...
class TestMLPlatformManager:
    ...
    def test_complete_multi_platform_workflow(self):
        ...
        # Log parameters
        params = {"model": "xgboost", "max_depth": 10, "learning_rate": 0.1}
        for platform_name, run_id in run_ids.items():
            manager.platforms[platform_name].log_parameters(run_id, params)

        # Log metrics
        manager.log_metrics_all(
            run_ids, {"accuracy": 0.96, "precision": 0.94, "recall": 0.95}
        )

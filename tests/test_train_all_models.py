...
def test_train_all_model_types():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for model_type in expected_types:
            assert (
                model_type in combined_output
            ), f"Model type {model_type} not found in output"
        print(f"✓ All {len(expected_types)} model types were trained")

        # Check for "Training all model types" message
        assert (
            "Training all model types" in combined_output
        ), "Missing 'Training all model types' message"
        print("✓ Training all model types message found")

        # Check training summary
        assert "TRAINING SUMMARY" in combined_output, "Missing training summary"
        assert (
            "Total models trained: 5" in combined_output
        ), "Wrong total models count in summary"
        ...
        ), f"Expected 5 models, got {summary['total_models']}"
        assert (
            summary["model_types"] == expected_types
        ), f"Model types mismatch: {summary['model_types']}"
        assert (
            summary["successful"] >= 5
        ), f"Expected all 5 models to succeed, got {summary['successful']}"
        print("✓ Audit summary verified")

        # Check results for each model
        assert "results" in summary, "Results not in summary"
        assert (
            len(summary["results"]) == 5
        ), f"Expected 5 results, got {len(summary['results'])}"
        ...
        for result_item in summary["results"]:
            ...
            print(
                f"✓ {model_type}: success={result_item['success']}, accuracy={accuracy_str}"
            )
...
def test_train_all_model_types_with_full_options():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            "Merkle audit logging enabled" in combined_output
        ), "Audit logging not enabled"
        print("✓ Audit logging enabled")

        assert (
            "Governance validation enabled" in combined_output
        ), "Governance not enabled"
        print("✓ Governance enabled")

        assert (
            "Ethical drift tracking enabled" in combined_output
        ), "Drift tracking not enabled"

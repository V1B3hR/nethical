...
class SoakTestMetrics:
    ...
    def get_memory_leak_analysis(self) -> Dict[str, Any]:
        ...
        initial_memory = sum(s['memory_mb'] for s in initial_samples) / len(initial_samples)
    ...
    def get_performance_analysis(self) -> Dict[str, Any]:
        ...
        if len(windows) >= 2:
            ...
            degradation_percent = (
                ...
                first_window['mean_response'] * 100
            ) if first_window['mean_response'] > 0 else 0
        else:
    ...
    def save_artifacts(self, output_dir: Path) -> Dict[str, str]:
        ...
        with open(md_file, 'w') as f:
            ...
            f.write(f"## Summary\n\n")
            f.write(f"- Duration: {stats['duration_hours']:.2f} hours ({stats['duration_seconds']:.0f} seconds)\n")
            ...
            if stats['memory_analysis']:
                ...
                f.write(f"- Growth: {mem['growth_mb']:.2f} MB ({mem['growth_percent']:+.2f}%)\n")
                f.write(f"- Max Memory: {mem['max_memory_mb']:.2f} MB\n")
                f.write(f"- Memory Leak Detected: {'❌ YES' if mem['leak_detected'] else '✅ NO'}\n\n")
            
            # Performance degradation analysis
            if stats['performance_analysis']:
                perf = stats['performance_analysis']
                f.write(f"## Performance Degradation Analysis\n\n")
                f.write(f"- Mean Response Time Degradation: {perf['degradation_percent']:+.2f}%\n")
                f.write(f"- Significant Degradation: {'❌ YES' if perf['degradation_detected'] else '✅ NO'}\n\n")
            ...
            if stats['performance_analysis']:
                passed = passed and not stats['performance_analysis']['degradation_detected']
            passed = passed and stats['success_rate'] >= 0.95
            
            f.write(f"## Test Result: {'✅ PASSED' if passed else '❌ FAILED'}\n\n")
            f.write("### Pass Criteria\n\n")
            f.write(f"- Memory growth < 5%: {'✅' if not stats['memory_analysis']['leak_detected'] else '❌'}\n")
            f.write(f"- Performance degradation < 20%: {'✅' if not stats['performance_analysis']['degradation_detected'] else '❌'}\n")
            f.write(f"- Success rate ≥ 95%: {'✅' if stats['success_rate'] >= 0.95 else '❌'}\n")
...
async def run_soak_test(governance: SafetyGovernance, duration_seconds: int, target_rps: int) -> SoakTestMetrics:
    """
    Run soak test
    ...
    while time.time() < end_time:
        ...
        if time.time() >= next_metric_time:
            ...
            print(f"  Progress: {elapsed/60:.1f} min / {duration_seconds/60:.1f} min ({elapsed/duration_seconds*100:.1f}%)")
...
async def test_soak_short(governance, output_dir):
    ...
    # Validate
    assert not mem['leak_detected'], f"Memory leak detected: {mem['growth_percent']:.2f}% growth"
    assert not perf['degradation_detected'], f"Performance degraded: {perf['degradation_percent']:.2f}%"
    assert stats['success_rate'] >= 0.95, f"Success rate too low: {stats['success_rate']*100:.2f}%"


@pytest.mark.asyncio
@pytest.mark.slow
@pytest.mark.skipif("not config.getoption('--run-soak')", reason="Soak tests not enabled")
async def test_soak_2hour(governance, output_dir):
    ...
    print(f"Memory growth < 5%: {'✅ PASS' if memory_pass else '❌ FAIL'} ({mem['growth_percent']:+.2f}%)")
    
    perf_pass = not perf['degradation_detected']
    print(f"Performance degradation < 20%: {'✅ PASS' if perf_pass else '❌ FAIL'} ({perf['degradation_percent']:+.2f}%)")
    
    success_pass = stats['success_rate'] >= 0.95
    print(f"Success rate ≥ 95%: {'✅ PASS' if success_pass else '❌ FAIL'} ({stats['success_rate']*100:.2f}%)")

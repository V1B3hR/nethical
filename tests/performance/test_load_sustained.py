...
class LoadTestMetrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "response_times": {
                ...
                "p50": (
                    sorted(self.response_times)[len(self.response_times) // 2]
                    if self.response_times
                    else 0
                ),
                "p95": (
    ...
    def save_artifacts(self, output_dir: Path):
        ...
        with open(md_file, "w") as f:
            ...
            f.write(
                f"- Memory Mean: {stats['system_metrics']['memory_mean_mb']:.2f} MB\n"
            )
            f.write(
                f"- Memory Max: {stats['system_metrics']['memory_max_mb']:.2f} MB\n\n"
            )
...
async def test_sustained_load_short(governance, output_dir):
    ...
    assert (
        stats["response_times"]["p95"] < 200
    ), f"P95 response time too high: {stats['response_times']['p95']:.2f}ms"
    assert (
        stats["throughput_rps"] >= 5
    ), f"Throughput too low: {stats['throughput_rps']:.2f} req/sec"
...
async def test_sustained_load_extended(governance, output_dir):
    ...
    assert (
        stats["response_times"]["p99"] < 500
    ), f"P99 response time too high: {stats['response_times']['p99']:.2f}ms"
    assert (
        stats["throughput_rps"] >= 15
    ), f"Throughput too low: {stats['throughput_rps']:.2f} req/sec"

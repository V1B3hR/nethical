...
class TestGovernancePerformance:
    ...
    def test_evaluate_action_performance(self, governance, sample_action):
        ...
        assert (
            stats["mean_ms"] < 100
        ), f"evaluate_action too slow: {stats['mean_ms']:.2f}ms"

        # Assert p95 is under 150ms
        assert (
            stats["p95_ms"] < 150
        ), f"evaluate_action p95 too slow: {stats['p95_ms']:.2f}ms"

        print(
            f"✓ evaluate_action performance: mean={stats['mean_ms']:.2f}ms, p95={stats['p95_ms']:.2f}ms"
        )
    ...
    def test_batch_evaluate_performance(self, governance):
        ...
        assert (
            stats["mean_ms"] < 500
        ), f"batch_evaluate too slow: {stats['mean_ms']:.2f}ms"

        print(
            f"✓ batch_evaluate performance: mean={stats['mean_ms']:.2f}ms for 10 actions"
        )
    ...
    def test_detector_initialization_performance(self):
        ...
        assert (
            stats["mean_ms"] < 50
        ), f"Initialization too slow: {stats['mean_ms']:.2f}ms"
...
class TestProfilerBaselines:
    ...
    def test_set_baselines(self, governance, sample_action):
        ...
        profiler.set_benchmark(
            name="evaluate_action", baseline_ms=stats["median_ms"], tolerance_pct=20.0
        )

        ...
        return {
            ...
            "degradation": {
                "mean_response_time": (
                    ...
                    if self.baseline_stats["response_times"]["mean"] > 0
                    else 0
                ),
                "p95_response_time": (
                    (
                    ...
                    if self.baseline_stats["response_times"]["p95"] > 0
                    else 0
                ),
                "success_rate": (
                    (
                        self.burst_stats["success_rate"]
                        - self.baseline_stats["success_rate"]
                    )
                    * 100
                ),
            },
    ...
    def save_artifacts(self, output_dir: Path) -> Dict[str, str]:
        ...
        with open(md_file, "w") as f:
            ...
            if comparison:
                ...
                passed = (
                    br["success_rate"] >= 0.90  # 90% success rate minimum
                    and deg["p95_response_time"] <= 300  # P95 degradation < 300%
                )
                f.write(f"## Test Result: {'✅ PASSED' if passed else '❌ FAILED'}\n\n")
                f.write("### Pass Criteria\n\n")
                f.write(
                    f"- Success rate ≥ 90%: {'✅' if br['success_rate'] >= 0.90 else '❌'} ({br['success_rate']*100:.2f}%)\n"
                )
                f.write(
                    f"- P95 degradation ≤ 300%: {'✅' if deg['p95_response_time'] <= 300 else '❌'} ({deg['p95_response_time']:.2f}%)\n"
                )
...
async def test_burst_load_5x(governance, output_dir):
    ...
    print(
        f"Success rate ≥ 90%: {'✅ PASS' if success_rate_pass else '❌ FAIL'} ({burst_stats['success_rate']*100:.2f}%)"
    )

    p95_degradation_pass = degradation["p95_response_time"] <= 300
    print(
        f"P95 degradation ≤ 300%: {'✅ PASS' if p95_degradation_pass else '❌ FAIL'} ({degradation['p95_response_time']:.2f}%)"
    )
    ...
    assert (
        success_rate_pass
    ), f"Success rate too low: {burst_stats['success_rate']*100:.2f}%"
    assert (
        p95_degradation_pass
    ), f"P95 degradation too high: {degradation['p95_response_time']:.2f}%"

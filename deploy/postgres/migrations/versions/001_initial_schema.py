...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "trust_level >= 0 AND trust_level <= 1", name="trust_level_range"
        ),
        sa.CheckConstraint(
            "status IN ('active', 'suspended', 'terminated', 'quarantine')",
            name="valid_status",
        ),
        schema="nethical",
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('staging', 'canary', 'production', 'deprecated', 'quarantine')",
            name="valid_model_status",
        ),
        schema="nethical",
    )

    op.create_index(
        "idx_model_versions_name", "model_versions", ["model_name"], schema="nethical"
    )
    op.create_index(
        "idx_model_versions_status", "model_versions", ["status"], schema="nethical"
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('quarantine', 'staging', 'active', 'deprecated')",
    ...
    op.create_index(
        "idx_policy_versions_status", "policy_versions", ["status"], schema="nethical"
    )
    ...
    op.create_index(
        ...
        "audit_events",
        ["decision", sa.text("time DESC")],
        schema="nethical",
    )
    op.create_index(
        "idx_audit_events_request", "audit_events", ["request_id"], schema="nethical"
    )
    ...
    op.create_index(
        "idx_api_keys_prefix", "api_keys", ["key_prefix"], schema="nethical"
    )

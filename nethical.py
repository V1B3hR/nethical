...
╚════════════════════════════════════════════════════════════╝
    ...
    confirm = input(
        Fore.YELLOW
        + "I have authorization to use and analyze these scans (yes/no): "
        + Style.RESET_ALL
    )
    ...
    try:
        ...
        for line in lines[-10:]:
            try:
                ...
                print(
                    f"{Fore.YELLOW}{timestamp}{Style.RESET_ALL} | {Fore.CYAN}{target}{Style.RESET_ALL} | {filelist} | {status_color}{status}{Style.RESET_ALL}"
                )
...
def generate_ai_report(
    scan_data: str, api_key: str, target: str, scan_files: list
) -> str:
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert cybersecurity analyst specializing in penetration testing and vulnerability assessment. Generate accurate, actionable reports based solely on supplied scan data.",
                },
...
"""
        ...
        print_colored(
            Fore.YELLOW, "[!] 'markdown' package not installed. Skipping HTML export."
        )
    except Exception as e:
        ...
        while True:
            ...
            if choice == "1":
                ...
                while True:
                    scan_input = input(
                        Fore.YELLOW
                        + "[?] Enter scan result filename (or press Enter to finish): "
                        + Style.RESET_ALL
                    ).strip()
                    if not scan_input:
                        break
                    ...
                    if not os.path.exists(clean_name):
                        print_colored(
                            Fore.RED,
                            f"[!] {clean_name} not found in current directory.",
                        )
                        continue
                ...
                target = input(
                    Fore.YELLOW
                    + "[?] Enter the target (domain or IP these scans refer to): "
                    + Style.RESET_ALL
                ).strip()
                if not target:
                ...
                for file in scan_files:
                    ...
                    if content:
                        ...
                        print_colored(
                            Fore.YELLOW,
                            f"[!] Warning: {file} could not be read or is empty.",
                        )

                if not full_scan_data:
                    print_colored(
                        Fore.RED,
                        "[!] None of the scan files had readable content. Aborting report generation.",
                    )
                    continue
                ...
                if not api_key:
                    api_key = input(
                        Fore.YELLOW
                        + "[?] Enter your OpenAI API key (or set OPENAI_API_KEY env variable): "
                        + Style.RESET_ALL
                    ).strip()
                if not api_key:
                    print_colored(
                        Fore.RED, "[!] API key is required to generate AI report."
                    )
                    continue
                ...
                try:
                    ...
                    print_colored(
                        Fore.GREEN,
                        f"[+] Report and artifacts will be saved in: {os.getcwd()}\n",
                    )
                except Exception as e:
                    print_colored(
                        Fore.RED, f"[!] Could not create results directory: {e}"
                    )
                    continue
                ...
                try:
                    print_colored(
                        Fore.CYAN,
                        f"[*] Contacting OpenAI (using model: {config.openai_model})...",
                    )
                    report_content = generate_ai_report(
                        full_scan_data, api_key, target, scan_files
                    )
                    ...
                    with open(report_filename, "w", encoding="utf-8") as f:
                        ...
                        f.write(
                            f"**Scan Date:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}\n\n"
                        )
                    ...
                    export_html = (
                        input(
                            Fore.YELLOW
                            + "[?] Export report to HTML as well? (y/n): "
                            + Style.RESET_ALL
                        )
                        .strip()
                        .lower()

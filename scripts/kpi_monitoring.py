...
class KPIMonitor:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = config_path or str(
            project_root / "config" / "kpi_config.json"
        )
    ...
    def _load_kpi_definitions(self) -> Dict:
        ...
        if os.path.exists(self.config_path):
            try:
                ...
                print(
                    f"Warning: Could not load KPI config from {self.config_path}: {e}"
                )
    ...
    def collect_kpis(self) -> Dict:
        ...
        kpi_values = {"timestamp": timestamp, "values": {}}
            ...
            try:
                with open(debt_register_path, "r") as f:
                    ...
                    return len(
                        [
                            d
                            for d in debt_register.get("items", [])
    ...
    def _calculate_trend(self, values: List[float]) -> Dict:
        ...
        return {"direction": direction, "magnitude": abs(pct_change)}
    ...
    def _generate_summary(self, current_kpis: Dict, trends: Dict) -> Dict:
        ...
        return {
            ...
            "critical_alerts": [
                kpi_id for kpi_id, v in values.items() if v.get("status") == "red"
            ],
        }
    ...
    def _format_text_report(self, report: Dict) -> str:
        ...
        for kpi_id, kpi_value in report["current_kpis"]["values"].items():
            ...
            lines.append(
                f"{status_symbol} {self.kpis[kpi_id]['name']}: "
                f"{kpi_value['value']} {kpi_value['unit']} "
        ...
        for kpi_id, trend in report["trends_30_days"]["trends"].items():
            if trend.get("direction") != "insufficient_data":
                ...
                lines.append(
                    f"{trend_symbol} {self.kpis[kpi_id]['name']}: "
                    f"{trend['direction']} "
...
def main():
    ...
    parser.add_argument("--config", help="Path to KPI configuration file")

    ...
    def migrate_policies(self) -> None:
        ...
        for policy_file in policies_path.glob("**/*.json"):
            try:
                ...
                if self.dry_run:
                    logger.info(
                        f"[DRY RUN] Would migrate policy: {policy_id} v{version}"
                    )
                    ...
                    batch.append(
                        {
                            ...
                            "action_content": entry.get("action_content")
                            or entry.get("action"),
        ...
        for model_dir in models_path.iterdir():
            ...
            for version_dir in model_dir.iterdir():
                ...
                try:
                    if self.dry_run:
                        logger.info(
                            f"[DRY RUN] Would migrate model: {model_name} v{version}"
                        )
    ...
    def migrate_artifacts_to_s3(self) -> None:
        ...
        for artifact_type_dir in artifacts_path.iterdir():
            ...
            for artifact_file in artifact_type_dir.glob("**/*"):
                ...
                try:
                    artifact_id = artifact_file.relative_to(
                        artifact_type_dir
                    ).as_posix()

                    if self.dry_run:
                        logger.info(
                            f"[DRY RUN] Would migrate artifact: {artifact_type}/{artifact_id}"
                        )
                    ...
                    if result:
                        logger.debug(
                            f"Migrated artifact: {artifact_type}/{artifact_id}"
                        )
                        self.stats["artifacts"]["migrated"] += 1
                    else:
                        logger.error(
                            f"Failed to migrate artifact: {artifact_type}/{artifact_id}"
                        )
    ...
    def migrate_audit_logs_to_s3(self) -> None:
        ...
        for date_str, log_files in logs_by_date.items():
            try:
                if self.dry_run:
                    logger.info(
                        f"[DRY RUN] Would archive {len(log_files)} logs for {date_str}"
                    )
                    continue
                ...
                result = self.s3.upload_audit_log(
                    date=date, log_id=log_id, data=compressed, compressed=True
                )

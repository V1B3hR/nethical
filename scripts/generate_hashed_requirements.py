...
class HashedRequirementsGenerator:
    ...
    def __init__(self, input_file: Path, output_file: Optional[Path] = None):
        ...
        self.output_file = (
            output_file or input_file.parent / f"{input_file.stem}-hashed.txt"
        )
    ...
    def get_package_hash(self, package_name: str, version: str) -> Optional[str]:
        ...
        try:
            ...
            print(
                f"Warning: Could not get hash for {package_name}=={version}: {e}",
                file=sys.stderr,
            )
        ...
        with open(self.output_file, "w") as f:
            ...
            for item in packages:
                if item["type"] == "comment":
                    ...
                    print(
                        f"Processing {package_name}=={version}...", end="", flush=True
                    )
                    ...
                    if hash_value:
                        ...
                        line = (
                            f"{package_name}=={version}  # WARNING: Hash not available"
                        )
                        if comment:
                            line += f" {comment}"
                        f.write(line + "\n")

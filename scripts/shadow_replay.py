...
class ShadowReplayTool:
    ...
    def __init__(
        ...
        if skip_methods is None:
            self.skip_methods = (
                [] if allow_modifying else ["POST", "PUT", "PATCH", "DELETE"]
            )
        else:
        ...
        logger.info("Shadow Replay Tool initialized:")
    ...
    def _check_production_safety(self) -> None:
        ...
        for pattern in self.PRODUCTION_DOMAINS:
            if pattern in target_lower:
                if not self.force:
                    logger.error(
                        f"SAFETY ERROR: Target URL contains production "
                        f"pattern '{pattern}': {self.staging_base_url}"
                    )
                    logger.error("Use --force flag to override (NOT RECOMMENDED)")
                    sys.exit(1)
                else:
    ...
    def _should_skip_request(
        ...
        # Note: skip_methods are normalized to uppercase in __init__
    ...
    def _send_request(self, request: ReplayRequest) -> ReplayResponse:
        """
    ...
    def replay_traffic(self, requests_list: List[ReplayRequest]) -> ReplayReport:
        ...
        for idx, request in enumerate(requests_list):
            ...
            if self.dry_run:
                logger.info(
                    f"[DRY-RUN] Would send: {staging_request.method} "
                    f"{staging_request.url}"
                )
                continue
  ...
  - Tool will abort if target URL looks like production (override with --force)
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default: POST PUT PATCH DELETE unless --allow-modifying)"
        ),
    )
    ...
    parser.add_argument(
        "--rps", type=float, help="Requests per second rate limit (optional)"
    )
    ...
    # Authentication
    parser.add_argument("--auth", help="Authorization token/key to use for requests")

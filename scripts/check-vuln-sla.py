...
def parse_vulnerability_file(filepath: str) -> Tuple[List[Dict], str]:
    ...
    if "Results" in data and any(
        "Vulnerabilities" in r for r in data.get("Results", [])
    ):
...
def check_sla(vulnerabilities: List[Dict]) -> Tuple[int, int, int]:
    ...
    for vuln in vulnerabilities:
        ...
        if not published_str:
            print(
                f"{YELLOW}Warning: No publish date for {vuln['id']}, skipping SLA check{NC}"
            )
            continue
    ...
    for breach in sorted(breaches, key=lambda x: x["age_hours"], reverse=True):
        ...
        print(
            f"Severity: {RED if breach['severity'] == 'CRITICAL' else YELLOW}{breach['severity']}{NC}"
        )
...
def main():
    ...
    if total_breaches == 0:
        ...
        if critical_breaches > 0:
            print(
                f"  {RED}• {critical_breaches} CRITICAL vulnerabilities exceed 24h SLA{NC}"
            )
        if high_breaches > 0:
            print(
                f"  {YELLOW}• {high_breaches} HIGH vulnerabilities exceed 72h SLA{NC}"
            )

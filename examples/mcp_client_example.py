...
class NethicalMCPClient:
    ...
    def check_pii(self, text: str, redact: bool = False) -> str:
        ...
        """
        result = self.call_tool("check_pii", {"text": text, "redact": redact})
    ...
    def check_violations(self, content: str, violation_types: list = None) -> str:
        ...
        result = self.call_tool(
            "check_violations",
            {"content": content, "violation_types": violation_types or []},
        )
...
'''
    eval_result = client.evaluate_action(
        action=safe_code, agent_id="demo_agent", action_type="code_generation"
    )
    print(eval_result)
    print()
...
"""
    eval_result = client.evaluate_action(
        action=pii_code, agent_id="demo_agent", action_type="code_generation"
    )
    print(eval_result)
    print()

...
def demo_sequence_anomaly():
    ...
    monitor = AnomalyDriftMonitor(
        sequence_n=3,
        storage_path="./demo_data/phase7_anomaly"
    )
...
def demo_alert_pipeline():
    ...
    monitor = AnomalyDriftMonitor(
        storage_path="./demo_data/phase7_alerts"
    )
...
def demo_synthetic_drift_test():
    ...
    caught_count = 0
    total_synthetic = sum(1 for _, _, _, should_detect in drift_scenarios if should_detect)
    ...
    for scenario_name, mean, std, should_detect in drift_scenarios:
        ...
        if detected:
            print(f"Drift detected (PSI={alert.evidence['psi_score']:.3f}, KL={alert.evidence['kl_divergence']:.3f})")
            if should_detect:
                caught_count += 1
        else:
    ...
    detection_rate = (caught_count / total_synthetic * 100) if total_synthetic > 0 else 0

...
async def demo_authentication():
    ...
    print(
        f"Session Token: {result.session_token[:20]}..." if result.session_token else ""
    )
...
async def demo_encryption():
    ...
    decrypted_decision = await encryption.decrypt_governance_decision(
        encrypted_decision, decision_id=decision["decision_id"]
    )
    ...
    print(
        f"NIST PQC standard: {quantum_eval['nist_pqc_standards']['key_encapsulation']}"
    )

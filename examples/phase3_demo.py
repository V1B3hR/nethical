...
def demo_risk_engine():
    ...
    print(f"   Agent 'agent_low': Risk Score = {score1:.3f}, Tier = {engine.get_tier('agent_low').value}")
    
    # Medium risk agent
    for _ in range(2):
        score2 = engine.calculate_risk_score("agent_medium", 0.5, {})
    print(f"   Agent 'agent_medium': Risk Score = {score2:.3f}, Tier = {engine.get_tier('agent_medium').value}")
    
    # High risk agent
    for _ in range(3):
        score3 = engine.calculate_risk_score("agent_high", 0.8, {})
    print(f"   Agent 'agent_high': Risk Score = {score3:.3f}, Tier = {engine.get_tier('agent_high').value}")
    
    print(f"\n2. Elevated tier triggers:")
    print(f"   agent_low should invoke advanced detectors? {engine.should_invoke_advanced_detectors('agent_low')}")
    print(f"   agent_high should invoke advanced detectors? {engine.should_invoke_advanced_detectors('agent_high')}")
    
    print(f"\n3. Risk tier transitions:")
    for tier in [RiskTier.LOW, RiskTier.NORMAL, RiskTier.HIGH, RiskTier.ELEVATED]:
        print(f"   {tier.value.upper()}: score >= {RiskTier.from_score(0.1 if tier == RiskTier.LOW else 0.3 if tier == RiskTier.NORMAL else 0.6 if tier == RiskTier.HIGH else 0.8)}")
...
def demo_fairness_sampling():
    ...
    population = {
        "cohort_a": [{"agent_id": f"agent_{i}", "action_id": f"action_{i}"} for i in range(15)],
        "cohort_b": [{"agent_id": f"agent_{i}", "action_id": f"action_{i}"} for i in range(10)],
        "cohort_c": [{"agent_id": f"agent_{i}", "action_id": f"action_{i}"} for i in range(5)],
    }
    ...
    for cohort, coverage in stats.get('cohort_coverage', {}).items():
        print(f"   {cohort}: {coverage['count']} samples ({coverage['percentage']:.1f}%)")
...
def demo_ethical_drift():
    ...
    print(f"   Tracked {sum(p.action_count for p in reporter.cohort_profiles.values())} actions")
    ...
    for cohort, summary in dashboard['cohort_summary'].items():
        print(f"   {cohort}: {summary['violation_count']} violations, "
...
def demo_integrated_governance():
    ...
    print(f"     Drift detected: {drift_report['drift_metrics'].get('has_drift', False)}")
    
    # Performance report
    perf_report = governance.get_performance_report()
    print(f"\n   Performance Report:")
    print(f"     Total actions: {perf_report['action_metrics']['total_actions']}")
    print(f"     CPU reduction: {perf_report['optimization']['current_cpu_reduction_pct']:.1f}%")

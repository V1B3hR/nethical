...
def demo_shadow_mode(gov):
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        print(f"Test {i}:")
        print(
            f"  Rule: {result['rule_risk_score']:.2f} ({result['rule_classification']})"
        )
        if "shadow" in result:
            shadow = result["shadow"]
            print(
                f"  ML Shadow: {shadow['ml_risk_score']:.2f} ({shadow['ml_classification']})"
            )
            print(
                f"  Agreement: Scores={shadow['scores_agree']}, "
    ...
    print(
        f"  Agreement Rate: {metrics.get('classification_agreement_rate', 0)*100:.1f}%"
    )

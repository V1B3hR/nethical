...
def demo_blended_risk():
    ...
    print(
        f"  Gray zone: [{blended_engine.gray_zone_lower}, {blended_engine.gray_zone_upper}]"
    )
    print(
        f"  Weights: {blended_engine.rule_weight} * rules + {blended_engine.ml_weight} * ML"
    )
    ...
    for zone_name, target_score, risk_level in zones_demo:
        ...
        print(
            f"     Rule Score: {decision.rule_risk_score:.3f} → {decision.rule_classification}"
        )
        print(f"     ML Score: {decision.ml_risk_score:.3f}")
        print(
            f"     Blended Score: {decision.blended_risk_score:.3f} → {decision.blended_classification}"
        )
    ...
    print(
        f"   - Clear Allow: {zone_dist['clear_allow']} ({zone_dist['clear_allow']/metrics['total_decisions']*100:.1f}%)"
    )
    print(
        f"   - Gray Zone: {zone_dist['gray_zone']} ({zone_dist['gray_zone_percentage']:.1f}%)"
    )
    print(
        f"   - Clear Deny: {zone_dist['clear_deny']} ({zone_dist['clear_deny']/metrics['total_decisions']*100:.1f}%)"
    )
    ...
    print(f"\n   Gate Criteria:")
    print(
        f"   - FP Delta: {gate_metrics['fp_delta']} ({gate_metrics['fp_delta_percentage']:.1f}%)"
    )
...
def demo_audit_trail():
    ...
    print(
        f"   - Formula: {decision.rule_weight}*{decision.rule_risk_score:.3f} + {decision.ml_weight}*{decision.ml_risk_score:.3f} = {decision.blended_risk_score:.3f}"
    )
    ...
    if decisions:
        ...
        print(
            f"   - Original: {audit['original_classification']} ({audit['rule_risk_score']:.3f})"
        )
        print(
            f"   - Final: {audit['final_classification']} ({audit['blended_risk_score']:.3f})"
        )
...
def demo_gray_zone_analysis():
    ...
    print(
        f"   - Classifications changed: {changed} ({changed/len(gray_zone_decisions)*100:.1f}%)"
    )

    escalated = sum(
        1 for d in gray_zone_decisions if d["blended_risk_score"] > d["rule_risk_score"]
    )

...
def demo_quarantine(gov):
    ...
    print(
        f"  Meets Requirement: {'✅ YES' if result['meets_requirement'] else '❌ NO'}"
    )
    ...
    action_result = gov.process_action(
        agent_id="agent_1", action="test_action", cohort="risky_cohort"
    )
...
def demo_sla_monitoring(gov):
    ...
    for i in range(100):
        gov.process_action(
            agent_id=f"agent_{i % 10}", action="test", cohort="performance_test"
        )
    ...
    print(f"\nLatency Metrics:")
    print(
        f"  P95: {report['p95_latency_ms']:.2f}ms (target: {report['p95_target_ms']}ms)"
    )

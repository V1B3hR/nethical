...
def demo_metrics_collection():
    ...
    for action_type, decision, region, latency in actions:
        ...
        print(f"   Recorded: {action_type} → {decision} (region: {region}, latency: {latency}s)")
    ...
    print(f"   Total violations: {sum(metrics['counters'].get('violations', {}).values())}")
...
def demo_tracing():
    ...
    with tracer.start_span("governance_check", attributes={"action": "api_call"}) as parent:
...
def demo_alert_rules():
    ...
    test_metrics = {
        'histograms': {
            'action_latency': {
                'p95': 6.5  # Exceeds critical threshold
            }
        },
        'gauges': {
            'error_rate': {
                'default': 0.08  # Exceeds 5% threshold
            },
            'quota_usage': {
                'agent_001': 0.93  # Exceeds 90% warning
            },
            'drift_score': {}
        }
    }

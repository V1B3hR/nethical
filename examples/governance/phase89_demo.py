...
def main():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "confidence": 0.55,
            "violations": [
                {"type": "privacy", "severity": 3, "description": "Privacy concern detected"}
            ]
        },
        {
    ...
    while True:
        ...
        # Case 3: Policy gap
            ...
            feedback = governance.submit_feedback(
                ...
                confidence=0.85,
                metadata={"suggested_policy": "Add explicit handling for cross-domain requests"}
            )
    ...
    print(f"  Median Resolution Time: {sla_metrics.median_resolution_time_seconds:.2f}s")
    ...
    print(f"  Gray Zone: [{baseline_config.gray_zone_lower}, {baseline_config.gray_zone_upper}]")
    ...
    for i, (config, metrics) in enumerate(results[:5], 1):
        ...
        print(f"       Recall: {metrics.detection_recall:.3f}, FP: {metrics.false_positive_rate:.3f}")
    ...
    if passed:
        ...
        if success:
            print(f"  ✓ Configuration {best_config.config_version} promoted to production")
        else:

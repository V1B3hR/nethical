...
try:
    ...
    print("Error: nethical package not found. Install with: pip install -e .", file=sys.stderr)
    sys.exit(1)
...
def main() -> int:
    ...
    parser.add_argument("--cohort", type=str, default="load-test", help="Cohort identifier")
    parser.add_argument("--storage-dir", type=str, default="./nethical_perf", help="Storage directory")
    ...
    parser.add_argument("--no-ml-blend", dest="enable_ml_blend", action="store_false", help="Disable ML blended enforcement")
    parser.add_argument("--anomaly", dest="enable_anomaly", action="store_true", default=False, help="Enable anomaly detection")
    parser.add_argument("--no-anomaly", dest="enable_anomaly", action="store_false", help="Disable anomaly detection")
    parser.add_argument("--merkle", dest="enable_merkle", action="store_true", default=True, help="Enable Merkle anchoring")
    parser.add_argument("--no-merkle", dest="enable_merkle", action="store_false", help="Disable Merkle anchoring")
    ...
    print(f"  p99 < 500ms:  {'✓ PASS' if p99_ok else '✗ FAIL'} ({stats['latency_p99_ms']:.2f}ms)")

...
def main():
    ...
    print("  2. Supports multiple preprocessing modes (heuristic, logistic, transformer)")
...
""")
    ...
    print("\n  Test Predictions:")
    print(f"  • Benign sample: label={result_benign['label']}, score={result_benign['score']:.3f}")
    print(f"  • Violation sample: label={result_violation['label']}, score={result_violation['score']:.3f}")
    ...
    print("   $ python training/train_any_model.py --model-type heuristic --num-samples 10000")
    
    print("\n2. Check saved model:")
    print("   $ ls -l models/candidates/heuristic_model_*.json")
    
    print("\n3. Enable governance for production:")
    print("   $ python training/train_any_model.py --model-type logistic --enable-governance --enable-audit")
    ...
    print("  • docs/implementation/TRAIN_MODEL_REAL_DATA_SUMMARY.md - Real data training")

...
def main():
    ...
    print(f"   Components enabled: {sum(status['components_enabled'].values())}/{len(status['components_enabled'])}")
    print()
    
    # Show phase-by-phase status
    print("   Phase 3 (Risk & Correlation):")
    print(f"      - Active risk profiles: {status['phase3']['risk_engine']['active_profiles']}")
    print(f"      - Tracked agents: {status['phase3']['correlation_engine']['tracked_agents']}")
    print()
    
    print("   Phase 4 (Audit & Taxonomy):")
    print(f"      - Merkle anchor enabled: {status['phase4']['merkle_anchor']['enabled']}")
    print(f"      - Quarantine manager enabled: {status['phase4']['quarantine_manager']['enabled']}")
    print()
    
    print("   Phase 5-7 (ML & Anomaly Detection):")
    print(f"      - Shadow classifier enabled: {status['phase567']['shadow_classifier']['enabled']}")
    print(f"      - ML blending enabled: {status['phase567']['blended_engine']['enabled']}")
    print(f"      - Anomaly monitoring enabled: {status['phase567']['anomaly_monitor']['enabled']}")
    print()
    
    print("   Phase 8-9 (Human & Optimization):")
    print(f"      - Pending escalation cases: {status['phase89']['escalation_queue']['pending_cases']}")
    print(f"      - Tracked configurations: {status['phase89']['optimizer']['tracked_configs']}")
    print()
    ...
    if 'shadow' in result2.get('phase567', {}):
        print(f"   ML shadow prediction: {result2['phase567']['shadow']['ml_risk_score']:.3f}")
        print(f"   Scores agree: {result2['phase567']['shadow']['scores_agree']}")
    
    if 'blended' in result2.get('phase567', {}):
        print(f"   Blended risk score: {result2['phase567']['blended']['blended_risk_score']:.3f}")
    ...
    if 'ethical_tags' in result2.get('phase4', {}):
        print(f"   Primary ethical dimension: {result2['phase4']['ethical_tags']['primary_dimension']}")
    print()
    
    # Final status
    print("5. Final System Status:")
    final_status = gov.get_system_status()
    print(f"   Total risk profiles: {final_status['phase3']['risk_engine']['active_profiles']}")
    print(f"   Total merkle events: {final_status['phase4']['merkle_anchor']['current_chunk_events']}")
    print(f"   Pending cases: {final_status['phase89']['escalation_queue']['pending_cases']}")
    print()

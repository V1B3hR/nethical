...
def demo_basic_explanation():
    ...
    result = governance.process_action(
        agent_id="demo_agent", action="Process customer analytics data"
    )
...
def demo_shap_values():
    ...
    # Evaluate action
    result = governance.process_action(
        agent_id="demo_agent", action="Delete user records without authorization"
    )
...
def demo_counterfactual():
    ...
    result = governance.process_action(
        agent_id="demo_agent", action="Execute system command: rm -rf /"
    )
...
def demo_visualization():
    ...
    # Evaluate action
    result = governance.process_action(
        agent_id="demo_agent", action="Access confidential financial records"
    )
    ...
    for child in sorted(
        viz_data["children"], key=lambda x: abs(x["impact"]), reverse=True
    ):
        impact_percent = child["impact"] * 100
        print(
            f"{child['name']:<20} Impact: {impact_percent:+6.2f}%  "
...
def demo_complete_workflow():
    ...
    explanation = explainer.explain_decision_with_features(
        decision=result["decision"], judgment_data=result
    )

    print("\n[2] Feature Importance (Top 3):")
    for feature in explanation.most_influential_features[:3]:
        f_obj = next(
            f for f in explanation.feature_importances if f.feature_name == feature
        )
    ...
    if result["decision"] != "ALLOW":
        counterfactual = explainer.generate_counterfactual(
            current_decision=result["decision"], judgment_data=result
        )
        print("\n[4] Counterfactual:")

...
class FinancialComplianceDetector(DetectorPlugin):
    ...
    def __init__(self):
        ...
        self.financial_keywords = [
            'financial_data', 'transaction', 'payment', 'credit_card',
            'bank_account', 'wire_transfer', 'investment', 'securities',
            'trading', 'portfolio', 'dividend', 'interest_rate'
        ]
    ...
    async def detect_violations(self, action: Any) -> Sequence[SafetyViolation] | None:
        ...
        detected_keywords = [kw for kw in self.financial_keywords 
    ...
    def _check_encryption(self, context: dict) -> bool:
        ...
        return context.get('encryption_enabled', False) or context.get('tls_enabled', False)
    
    def _check_authorization(self, context: dict) -> bool:
        """Check if proper authorization is present."""
        return (context.get('authorized', False) or 
                context.get('user_role') in ['admin', 'financial_officer'])
...
class HealthcareComplianceDetector(DetectorPlugin):
    ...
    def __init__(self):
        ...
        super().__init__(
            name="HealthcareComplianceDetector",
            version="1.0.0"
        )
        ...
        self.healthcare_keywords = [
            'patient', 'diagnosis', 'treatment', 'prescription', 'medical_record',
            'phi', 'hipaa', 'health_information', 'medical_history', 'clinical'
        ]
    ...
    async def detect_violations(self, action: Any) -> Sequence[SafetyViolation] | None:
        ...
        detected_keywords = [kw for kw in self.healthcare_keywords
                            if kw.replace('_', ' ') in content.lower()]
    ...
    def _check_security_measures(self, context: dict) -> bool:
        ...
        return (context.get('encryption_enabled', False) and 
                context.get('audit_logging', False))
...
class CustomPolicyDetector(DetectorPlugin):
    ...
    def __init__(self, policy_name: str = "custom_policy", 
        ...
        """
        super().__init__(
            name=f"CustomPolicyDetector_{policy_name}",
            version="1.0.0"
        )

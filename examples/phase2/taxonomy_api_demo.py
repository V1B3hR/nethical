...
def main():
    ...
    if schema_result["success"]:
        ...
        print(
            f"   Required fields: {', '.join(schema_result['data']['schema']['required'])}"
        )
    ...
    if healthcare_result["success"]:
        ...
        for dim_name in ["patient_safety", "medical_privacy"]:
            if dim_name in taxonomy["dimensions"]:
                print(
                    f"     - {dim_name}: {taxonomy['dimensions'][dim_name]['description']}"
                )
    ...
    if tag_result["success"]:
        ...
        for tag in tag_result["data"]["tags"]:
            print(
                f"     - {tag['dimension']}: {tag['score']:.2f} (confidence: {tag['confidence']:.2f})"
            )

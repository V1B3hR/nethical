...
def example_rbac_basic():
    ...
    # Check permissions
    print("\n2. Permission Checks:")
    print(f"   Can Alice manage users? {rbac.has_permission('alice', Permission.MANAGE_USERS)}")
    print(f"   Can Bob execute actions? {rbac.has_permission('bob', Permission.EXECUTE_ACTIONS)}")
    print(f"   Can Charlie execute actions? {rbac.has_permission('charlie', Permission.EXECUTE_ACTIONS)}")
    
    # Check role hierarchy
    print("\n3. Role Hierarchy:")
    print(f"   Does Alice have operator privileges? {rbac.has_role('alice', Role.OPERATOR)}")
...
def example_authentication():
    ...
    access_token, access_payload = auth.create_access_token("user123", scope="read write")
...
def example_api_keys():
    ...
    print(f"   Key string: {api_key_string[:20]}...{api_key_string[-10:]}")

...
class HumanReviewQueue:
    ...
    def assign_item(self, item_id: str, reviewer_id: str) -> bool:
        ...
        if item.status != ReviewStatus.PENDING:
            logger.warning(
                f"Item {item_id} is not pending (status: {item.status.value})"
            )
            return False
    ...
    def complete_item(
        self, item_id: str, feedback_category: FeedbackCategory, feedback_text: str
    ) -> bool:
        """
    ...
    def get_sla_metrics(self) -> SLAMetrics:
        ...
        pending_items = sum(
            1 for item in self._items.values() if item.status == ReviewStatus.PENDING
        )
        in_progress = sum(
            1
        ...
        within_sla = sum(
            ...
            and item.sla_deadline
            and item.completed_at <= item.sla_deadline
        )

        sla_compliance_rate = (
            (within_sla / completed_items * 100.0) if completed_items > 0 else 100.0
        )
    ...
    def get_reviewer_stats(self, reviewer_id: str) -> ReviewerStats:
        ...
        reviewer_items = [
            self._items[iid] for iid in reviewer_item_ids if iid in self._items
        ]

        completed = [
            item for item in reviewer_items if item.status == ReviewStatus.COMPLETED
        ]
        ...
        feedback_dist = Counter(
            item.feedback_category.value for item in completed if item.feedback_category
        )
    ...
    def _calculate_reviewer_drift(
        self, reviewer_id: str, completed_items: List[ReviewItem]
    ) -> float:
        """
    ...
    def get_drift_report(self) -> Dict[str, Any]:
        ...
        max_drift = max((s["drift_score"] for s in drift_stats.values()), default=0.0)
        avg_drift = (

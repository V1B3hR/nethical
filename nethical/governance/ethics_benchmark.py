...
class BenchmarkMetrics:
    ...
    def meets_targets(self, 
                     ...
                     max_fpr: float = 0.05,
                     max_fnr: float = 0.08) -> Tuple[bool, List[str]]:
        ...
        if self.precision < min_precision:
            passed = False
            reasons.append(f"Precision {self.precision:.3f} below target {min_precision}")
...
class EthicsBenchmark:
    ...
    def evaluate(self, 
                detector_fn,
                per_type: bool = True) -> BenchmarkMetrics:
    ...
    def generate_report(self, 
        ...
        if metrics.per_type_metrics:
            ...
            lines.append("-" * 70)
            lines.append(f"{'Type':<20} {'Precision':<12} {'Recall':<12} {'F1':<12} {'Support':<10}")
            lines.append("-" * 70)

...
class ThresholdVersionManager:
    ...
    def __init__(self, storage_dir: str = "governance/thresholds"):
        ...
        if not str(self.storage_dir).startswith(str(Path.cwd().resolve())):
            ...
            if not str(self.storage_dir).startswith("/tmp"):
                raise ValueError(
                    f"Storage directory must be within current directory: {storage_dir}"
                )
    ...
    def get_threshold(
        self, threshold_name: str, version: Optional[str] = None
    ) -> Optional[Threshold]:
    ...
    def evaluate_thresholds(
        self, values: Dict[str, float], version: Optional[str] = None
    ) -> Dict[str, bool]:

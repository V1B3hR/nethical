...
class Benchmark:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_avg_ms': statistics.mean(self.samples) if self.samples else None,
            'recent_std_ms': statistics.stdev(self.samples) if len(self.samples) > 1 else None
        }
...
class PerformanceProfiler:
    ...
    def __init__(self, results_dir: Union[str, Path] = "profiling_results"):
    ...
    def profile(self, func: Optional[Callable] = None, *, name: Optional[str] = None, detailed: bool = False):
        """
            ...
            def wrapper(*args, **kwargs):
                if detailed:
                    ...
                    profile_result = ProfileResult(
                        ...
                        call_count=1,
                        stats={'profile_output': stats_str[:1000]}  # Limit size
                    )
                else:
                    # Simple timing
                    start_time = time.perf_counter()
                ...
                if func_name in self.benchmarks:
                    ...
                    if is_regression:
                        print(f"[WARNING] Performance regression detected for {func_name}: "
                              f"{pct_change:.1f}% slower than baseline "
                              f"({execution_time:.2f}ms vs {benchmark.baseline_ms:.2f}ms)")
    ...
    def benchmark_function(self, func: Callable, iterations: int = 100, **kwargs) -> Dict[str, Any]:
        """
        ...
        print(f"Results: mean={stats['mean_ms']:.2f}ms, median={stats['median_ms']:.2f}ms, "
    ...
    def set_benchmark(self, name: str, baseline_ms: float, tolerance_pct: float = 10.0):
        ...
        print(f"Set benchmark '{name}': {baseline_ms:.2f}ms (tolerance: ±{tolerance_pct}%)")
    ...
    def _save_benchmarks(self):
        ...
        benchmarks_file = self.results_dir / "benchmarks.json"
...
def profile(func: Optional[Callable] = None, *, name: Optional[str] = None, detailed: bool = False):
    """Convenience decorator using global profiler"""
    return _profiler.profile(func, name=name, detailed=detailed)

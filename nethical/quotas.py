...
class QuotaUsage:
    ...
    action_times: deque = field(default_factory=lambda: deque(maxlen=5000))
...
class QuotaEnforcer:
    ...
    def __init__(self, config: Optional[QuotaConfig] = None):
        ...
        logger.info(f"QuotaEnforcer initialised with config: {self.config}")
    ...
    def check_quota(
        ...
        with self._lock:
            ...
            agent_check = self._check_entity_quota(
                agent_id, "agent", current_time, payload_size, action_type=action_type
            )
            ...
            result["backpressure_level"] = max(
                result["backpressure_level"], agent_check["backpressure_level"]
            )
            if agent_check["decision"] == "THROTTLE":
                result["decision"] = "THROTTLE"
                result["enforcement_action"] = agent_check["enforcement_action"]
                result["reason"] = agent_check["reason"]

            # Check cohort quota if enabled
            if self.config.enable_cohort_isolation and cohort:
                cohort_check = self._check_entity_quota(
                    cohort, "cohort", current_time, payload_size, action_type=action_type
                )
                ...
                result["backpressure_level"] = max(
                    result["backpressure_level"], cohort_check["backpressure_level"]
                )
                if cohort_check["decision"] == "THROTTLE":
                    result["decision"] = "THROTTLE"
                    result["enforcement_action"] = cohort_check["enforcement_action"]
                    result["reason"] = cohort_check["reason"]

            # Check tenant quota if enabled
            if self.config.enable_tenant_isolation and tenant:
                tenant_check = self._check_entity_quota(
                    tenant, "tenant", current_time, payload_size, action_type=action_type
                )
                ...
                result["backpressure_level"] = max(
                    result["backpressure_level"], tenant_check["backpressure_level"]
                )
                if tenant_check["decision"] == "THROTTLE":
                    result["decision"] = "THROTTLE"
                    result["enforcement_action"] = tenant_check["enforcement_action"]
                    result["reason"] = tenant_check["reason"]
            ...
            # Record usage
            self._record_usage(agent_id, "agent", current_time, payload_size, action_type=action_type)
            if cohort:
                self._record_usage(cohort, "cohort", current_time, payload_size, action_type=action_type)
            if tenant:
                self._record_usage(tenant, "tenant", current_time, payload_size, action_type=action_type)
    ...
    def _check_entity_quota(
        ...
        rate_window = max(float(self.config.rate_window_seconds), 1.0)
        request_rate = len(recent_requests) / rate_window
        ...
        rate_utilization = request_rate / max(self.config.requests_per_second, 1e-6)
        burst_utilization = burst_size / max(float(self.config.burst_size), 1.0)
        overall_utilization = max(rate_utilization, burst_utilization)

        # Calculate action rate if applicable
        action_rate = 0.0
        if action_type != "query" and self.config.actions_per_minute > 0:
            action_window_start = current_time - 60.0
            recent_actions = [t for t in usage.action_times if t >= action_window_start]
            action_rate = float(len(recent_actions))
            action_utilization = action_rate / max(float(self.config.actions_per_minute), 1.0)
            overall_utilization = max(overall_utilization, action_utilization)
        ...
        result = {
            ...
            "usage": {
                ...
                "total_actions": usage.actions_count,
        ...
        if overall_utilization >= self.config.block_threshold:
            ...
            result["decision"] = "BLOCK"
            result["reason"] = (
                f"{entity_type} {entity_id} exceeded quota: {overall_utilization:.1%} utilization"
            )
            ...
            result["decision"] = "THROTTLE"
            result["reason"] = (
                f"{entity_type} {entity_id} approaching quota: {overall_utilization:.1%} utilization"
            )
    ...
    def _record_usage(
        ...
        if action_type != "query":
            usage.actions_count += 1
            usage.action_times.append(current_time)
    ...
    def get_usage_summary(self, entity_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_requests = [t for t in usage.request_times if t >= window_start]
            rate_window = max(float(self.config.rate_window_seconds), 1.0)
            current_rate = len(recent_requests) / rate_window
            ...
            return {
                ...
                "first_request": (
                    datetime.fromtimestamp(usage.first_request_time, tz=timezone.utc).isoformat()
                    if usage.first_request_time
                    else None
                ),
                "last_request": (
                    datetime.fromtimestamp(usage.last_request_time, tz=timezone.utc).isoformat()
                    if usage.last_request_time
                    else None
                ),
                "last_violation": (
                    datetime.fromtimestamp(usage.last_violation_time, tz=timezone.utc).isoformat()
                    if usage.last_violation_time
                    else None
                ),
            }

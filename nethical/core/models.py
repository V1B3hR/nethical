...
class SafetyViolation(_BaseModel):
    ...
    def needs_immediate_action(self) -> bool:
        ...
        return (
            self.is_critical and 
            self.confidence >= 0.7 and 
            not self.remediation_applied and
...
class JudgmentResult(_BaseModel):
    ...
    violations: List[SafetyViolation] = Field(
        ...
        description="Violations considered"
    )
    modifications: Dict[str, Any] = Field(
        default_factory=dict, 
        description="Modifications to apply to the action"
    )
    feedback: List[str] = Field(default_factory=list, description="Actionable feedback")
    timestamp: datetime = Field(default_factory=now_tz, description="Timestamp (TZ-aware)")
    remediation_steps: List[str] = Field(
        default_factory=list, 
        description="Steps to remediate issues"
    )
...
class MonitoringConfig(_BaseModel):
    ...
    risk_threshold: float = Field(
        ...
        description="Threshold for risk scoring"
    )
    ...
    enable_manipulation_detection: bool = Field(default=True, description="Enable manipulation detection")
    ...
    enable_adversarial_detection: bool = Field(default=True, description="Enable adversarial detection")
    enable_real_time_monitoring: bool = Field(default=True, description="Enable real-time processing")
...
def export_judgment_report(judgment: JudgmentResult) -> Dict[str, Any]:
    ...
    return {
        ...
        "max_severity": judgment.max_violation_severity.name if judgment.max_violation_severity else None,

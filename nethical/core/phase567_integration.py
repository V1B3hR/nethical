...
class Phase567IntegratedGovernance:
    ...
    def export_phase567_report(self) -> str:
        ...
        if self.shadow_classifier:
            ...
            lines.append(f"- Agreement Rate: {metrics.get('classification_agreement_rate', 0.0)*100:.1f}%")
            lines.append("")
        ...
        if self.anomaly_monitor:
            ...
            lines.append(f"- Distributional Anomalies: {stats['alerts']['by_type'].get('distributional', 0)}")
            lines.append(f"- Critical Severity: {stats['alerts']['by_severity'].get('critical', 0)}")
            lines.append("")

...
class SLAMonitor:
    ...
    def _cleanup_window(self):
        ...
        while (self.latency_window.timestamps and 
               self.latency_window.timestamps[0] < cutoff):
    ...
    def check_sla_compliance(self) -> Dict[str, Any]:
        ...
        for target_name, target in self.sla_targets.items():
            ...
            breach_pct = ((actual_value - target_value) / target_value * 100) if actual_value > target_value else 0.0
    ...
    def get_sla_report(self) -> Dict[str, Any]:
        ...
        breach_count = len([b for b in self.breaches 
        ...
        return {
            ...
            'p95_margin_pct': ((p95_target - p95_actual) / p95_target * 100) if p95_target > 0 else 0,
            'metrics': metrics,
    ...
    def get_breach_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            'breaches_by_metric': {
                metric: len(breaches)
                for metric, breaches in breaches_by_metric.items()
            },
            'most_recent': {
    ...
    def export_sla_documentation(self) -> str:
        ...
        lines.append(f"- **P95 Latency**: {report['p95_latency_ms']:.1f}ms (target: {report['p95_target_ms']}ms)")

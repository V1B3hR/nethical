...
# Each primitive has:  base keywords, synonyms, technical terms, and patterns
PRIMITIVE_KEYWORDS = {
    SemanticPrimitive.ACCESS_USER_DATA: {
        "base":  ["access", "read", "get", "fetch", "retrieve", "query", "view", "check"],
        "data_terms": ["user data", "personal", "profile", "account", "preferences", "settings"],
        "database": ["select", "find", "search", "lookup", "pull", "extract"],
        "privacy": ["pii", "personal information", "sensitive data", "private"],
    },
    
    SemanticPrimitive. MODIFY_USER_DATA: {
        "base": ["modify", "update", "change", "edit", "alter", "revise", "set"],
        "data_terms":  ["user data", "profile", "account", "preferences", "settings"],
        "database": ["update", "patch", "put", "post", "insert", "upsert"],
    },
    
    SemanticPrimitive.DELETE_USER_DATA: {
        "base":  ["delete", "remove", "erase", "purge", "clear", "wipe", "destroy"],
        "data_terms":  ["user data", "profile", "account", "record"],
        "database": ["drop", "truncate"],
        "privacy": ["forget", "right to be forgotten"],
    },
    
    SemanticPrimitive. SHARE_USER_DATA: {
        "base": ["share", "send", "transmit", "export", "transfer", "forward"],
        "data_terms":  ["user data", "personal", "information"],
        "network": ["publish", "broadcast", "distribute", "disclose"],
    },
    
    SemanticPrimitive.EXECUTE_CODE: {
        "base": ["execute", "run", "eval", "exec", "invoke", "call", "launch"],
        "code_terms":  ["code", "script", "program", "command", "function", "method"],
        "system": ["subprocess", "shell", "terminal", "process"],
        "dangerous": ["os.system", "subprocess.call", "eval(", "exec("],
    },
    
    SemanticPrimitive.GENERATE_CODE: {
        "base": ["generate", "create", "write", "produce", "build", "compose"],
        "code_terms":  ["code", "function", "class", "method", "script", "program"],
        "patterns": ["def ", "class ", "function", "lambda", "import "],
    },
    
    SemanticPrimitive.MODIFY_CODE: {
        "base": ["modify", "edit", "change", "refactor", "update", "rewrite"],
        "code_terms": ["code", "function", "class", "implementation"],
    },
    
    SemanticPrimitive.ACCESS_SYSTEM: {
        "base": ["access", "read", "open", "view"],
        "system_terms": ["system", "os", "file", "filesystem", "directory", "path"],
        "resources": ["process", "thread", "memory", "cpu", "disk"],
    },
    
    SemanticPrimitive.MODIFY_SYSTEM: {
        "base": ["modify", "change", "configure", "set", "install", "uninstall"],
        "system_terms": ["system", "config", "settings", "environment", "registry"],
        "files": ["file", "directory", "folder", "permissions", "chmod"],
        "dangerous": ["format", "partition", "mount", "unmount"],
    },
    
    SemanticPrimitive.NETWORK_ACCESS: {
        "base": ["network", "connect", "request", "call"],
        "protocols": ["http", "https", "ftp", "ssh", "tcp", "udp", "websocket"],
        "operations": ["api", "request", "fetch", "download", "upload", "get", "post"],
        "domains": ["url", "endpoint", "server", "host", "domain"],
    },
    
    SemanticPrimitive. GENERATE_CONTENT: {
        "base": ["generate", "create", "write", "compose", "produce", "draft"],
        "content_types": ["content", "text", "document", "article", "message", "response"],
        "media": ["image", "video", "audio", "media"],
    },
    
    SemanticPrimitive.ANALYZE_CONTENT: {
        "base":  ["analyze", "examine", "inspect", "review", "evaluate", "assess"],
        "content_types": ["content", "text", "document", "data", "input"],
        "nlp": ["parse", "tokenize", "sentiment", "classify"],
    },
    
    SemanticPrimitive.TRANSFORM_CONTENT: {
        "base": ["transform", "convert", "translate", "process", "modify"],
        "operations": ["format", "encode", "decode", "compress", "encrypt"],
    },
    
    SemanticPrimitive.MAKE_DECISION: {
        "base":  ["decide", "decision", "choose", "select", "determine", "conclude"],
        "logic": ["if", "else", "when", "condition", "rule", "policy"],
        "ai": ["infer", "predict", "recommend", "suggest"],
    },
    
    SemanticPrimitive.PROVIDE_RECOMMENDATION: {
        "base": ["recommend", "suggest", "advise", "propose", "offer"],
        "content":  ["recommendation", "suggestion", "advice", "guidance"],
    },
    
    SemanticPrimitive.COMMUNICATE_WITH_USER: {
        "base": ["tell", "inform", "notify", "alert", "message", "communicate"],
        "actions": ["say", "respond", "reply", "answer", "report"],
        "channels": ["email", "chat", "notification", "alert"],
    },
    
    SemanticPrimitive.COMMUNICATE_WITH_SYSTEM: {
        "base":  ["communicate", "interface", "interact", "call"],
        "targets": ["system", "service", "api", "component", "module"],
    },
    
    SemanticPrimitive.UPDATE_MODEL:  {
        "base": ["update", "train", "retrain", "fine-tune", "calibrate"],
        "ml_terms": ["model", "weights", "parameters", "neural network"],
        "operations": ["fit", "learn", "optimize", "converge"],
    },
    
    SemanticPrimitive. LEARN_FROM_DATA: {
        "base": ["learn", "adapt", "improve", "evolve"],
        "data_terms": ["data", "examples", "samples", "training set"],
        "ml_terms": ["feedback", "reinforcement", "supervised", "unsupervised"],
    },
    
    SemanticPrimitive.PHYSICAL_MOVEMENT: {
        "base": ["move", "navigate", "go to", "travel", "drive"],
        "robot_terms": ["robot", "actuator", "motor", "servo"],
        "directions": ["forward", "backward", "left", "right", "up", "down"],
        "locations": ["position", "coordinates", "destination", "waypoint"],
    },
    
    SemanticPrimitive.PHYSICAL_MANIPULATION: {
        "base": ["grasp", "grab", "pick", "place", "manipulate", "handle"],
        "robot_terms": ["gripper", "effector", "arm", "hand"],
        "actions": ["push", "pull", "lift", "rotate", "twist"],
    },
    
    SemanticPrimitive.EMERGENCY_STOP:  {
        "base": ["stop", "emergency", "halt", "abort", "kill", "shutdown"],
        "safety": ["e-stop", "emergency stop", "panic", "terminate"],
        "critical": ["immediate", "urgent", "critical"],
    },
}
...
class EnhancedPrimitiveDetector:
    ...
    def __init__(
        ...
        self.use_embedding_similarity = use_embedding_similarity and embedding_engine is not None
    ...
    def _precompute_primitive_embeddings(self):
        ...
        for primitive, description in primitive_descriptions. items():
            try:
                emb = self.embedding_engine. embed(description, metadata={"primitive": primitive. value})
    ...
    def _detect_by_keywords(self, text_lower: str) -> Set[SemanticPrimitive]:
        ...
        for primitive, keyword_groups in PRIMITIVE_KEYWORDS.items():
            # Check all keyword groups for this primitive
    ...
    def _detect_by_patterns(self, text_lower: str, original_text: str) -> Set[SemanticPrimitive]: 
    ...
    def _detect_by_embedding_similarity(self, action_text: str) -> Set[SemanticPrimitive]: 

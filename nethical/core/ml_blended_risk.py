...
class BlendingMetrics:
    ...
    def gate_check(self, max_fp_delta_pct: float = 5.0) -> Tuple[bool, str]:
        ...
        if abs(self.fp_delta_percentage) > max_fp_delta_pct:
            return False, f"FP delta {self.fp_delta_percentage:.1f}% exceeds limit {max_fp_delta_pct}%"
...
class MLBlendedRiskEngine:
    ...
    def compute_blended_risk(
        ...
        if risk_zone == RiskZone.GRAY_ZONE and self.enable_ml_blending and ml_risk_score is not None:
            # Blend in gray zone
            decision.blended_risk_score = (
                self.rule_weight * rule_risk_score +
            ...
            decision.explanation = f"Gray zone: blended {self.rule_weight}*rule + {self.ml_weight}*ML"
        else:

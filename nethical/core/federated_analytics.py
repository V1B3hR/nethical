...
class FederatedAnalytics:
    ...
    def compute_metrics(
        ...
        for metric_name, values_and_sizes in regional_values.items():
            ...
            if privacy_preserving:
                aggregated[metric_name] = self._add_privacy_noise(
                    aggregated[metric_name], noise_level
                )
        ...
        result = AggregatedMetrics(
            ...
            metadata={"min_samples_per_region": self.min_samples_per_region},
        )
    ...
    def _compute_correlation_p_value(self, correlation: float, n: int) -> float:

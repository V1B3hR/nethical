...
class HTTPWebhookDispatcher(WebhookDispatcher):
    ...
    def _should_retry(self, status_code: Optional[int], err: Optional[BaseException], attempt: int) -> bool:
    ...
    def _compute_delay(self, base: float, attempt: int, retry_after: Optional[float] = None) -> float:
    ...
    def dispatch(self, payload: WebhookPayload) -> WebhookDelivery:
        ...
        if self.dry_run:
            ...
            logging.info(f"[DRY-RUN] Webhook delivery simulated: {delivery.delivery_id} -> {delivery.url}")
            if self.on_success:
                try:
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                with request.urlopen(req, timeout=self.timeout) as response:
                    ...
                    if 200 <= response.getcode() < 300:
                        delivery.status = WebhookStatus.SUCCESS
                        logging.info(f"Webhook delivered successfully: {delivery.delivery_id} ({response.getcode()})")
                        break
                    else:
                        delivery.status = WebhookStatus.FAILED
                        delivery.error_message = f"HTTP {response.getcode()}"
                        logging.warning(f"Webhook non-2xx response: {delivery.delivery_id} ({response.getcode()})")

            except error.HTTPError as e:
                duration = (time.monotonic() - start) * 1000.0
                delivery.duration_ms = round(duration, 3)
                delivery.response_code = e.code
                delivery.response_headers = self._headers_to_dict(e.headers) if hasattr(e, "headers") and e.headers else None
                ...
                logging.error(f"Webhook failed (HTTP {e.code}): {delivery.delivery_id} - {e.reason}")
                ...
                logging.error(f"Webhook failed (URL error): {delivery.delivery_id} - {delivery.error_message}")
            ...
            # Retry delay
            if attempt < self.max_retries:
                sleep_for = self._compute_delay(self.retry_delay, attempt, retry_after=retry_after_secs)
                logging.debug(f"Retrying webhook {delivery.delivery_id} in {sleep_for:.2f}s (attempt {attempt + 1}/{self.max_retries})")
...
class SlackWebhookDispatcher(HTTPWebhookDispatcher):
    ...
    def send_message(self,
                     ...
                     blocks: Optional[List[Dict]] = None) -> WebhookDelivery:
        """
    ...
    def send_alert(self,
                   ...
                   use_blocks: bool = True) -> WebhookDelivery:
        """
        ...
        if use_blocks:
            ...
            blocks.append({
                "type": "context",
                "elements": [
                    {"type": "mrkdwn", "text": f"Timestamp: `{datetime.now(timezone.utc).isoformat()}`"}
                ]
            })
...
class DiscordWebhookDispatcher(HTTPWebhookDispatcher):
    ...
    def send_message(self,
                     ...
                     avatar_url: Optional[str] = None) -> WebhookDelivery:
        """
...
class GenericAPIIntegration(APIIntegration):
    ...
    def send_data(self, data: Dict[str, Any]) -> bool:
        """Send data to API"""
        # Stub implementation
        logging.info(f"[STUB] Would send data to {self.base_url}: {json.dumps(data, sort_keys=True)[:100]}...")
        return True
...
class WebhookManager:
    ...
    def send_to(self, webhook_name: str, event_type: str, data: Dict[str, Any], **metadata) -> Optional[WebhookDelivery]:

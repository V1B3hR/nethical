...
class AdvancedManipulationEngine:
    ...
    def _analyze_pattern_subcategory(self, content: str, category: str, subcategory: str, patterns: List[Pattern], results: Dict[str, Any]):
        matches = []
        for pattern in patterns:
    ...
    def _analyze_linguistic_features(self, content: str) -> Dict[str, Any]:
        ...
        features = {
            ...
            'imperative_count': sum(1 for word in words if word.lower() in ['must', 'should', 'need']),
            'imperative_ratio': sum(1 for word in words if word.lower() in ['must', 'should', 'need']) / max(len(words), 1),
            'question_count': len(re.findall(r'\?', content)),
            'question_ratio': len(re.findall(r'\?', content)) / max(len([s for s in sentences if s.strip()]), 1)
        }
        return features
    ...
    def _analyze_emotional_markers(self, content: str) -> Dict[str, float]:
        content_lower = content.lower()
        markers = {
            'fear_score': min(sum(1 for word in ['afraid', 'scared', 'panic'] if word in content_lower) * 0.2, 1.0),
            'urgency_score': min(sum(1 for word in ['urgent', 'immediate', 'now'] if word in content_lower) * 0.15, 1.0),
            'vulnerability_targeting_score': min(sum(1 for word in ['alone', 'helpless', 'vulnerable'] if word in content_lower) * 0.25, 1.0),
            'scarcity_score': min(sum(1 for phrase in ['exclusive', 'limited', 'rare', 'only'] if phrase in content_lower) * 0.25, 1.0)
        }
        return markers
    ...
    def _calculate_cognitive_load(self, content: str, results: Dict[str, Any]) -> Dict[str, float]:
        ...
        indicators = {
            ...
            'decision_pressure': min(sum(len(matches) for matches in results['pattern_matches'].values()) * 0.1, 1.0)
        }
        return indicators
...
class EnhancedDarkPatternDetector:
    ...
    def _validate_input(self, content: str) -> bool:
        if not content or not isinstance(content, str): return False
        if len(content) > self.max_content_length:
            logger.warning(f"Content exceeds maximum length: {len(content)}")
            return False
        suspicious_patterns = [r'<script[^>]*>.*?</script>', r'javascript:', r'data:text/html', r'vbscript:']
        for pattern in suspicious_patterns:
            if re.search(pattern, content, re.IGNORECASE): return False
        return True
    ...
    async def _assess_user_vulnerability(self, content: str, context: Optional[Dict[str, Any]]) -> Dict[str, float]:
        ...
        if not context: return vulnerability_scores
    ...
    def _calculate_sophistication_score(self, pattern_analysis: Dict[str, Any]) -> float:
        ...
        if not sophistication_scores: return 0.0
        weighted_scores = []
    ...
    async def _generate_detection_results(self, analysis_results: Dict[str, Any], action: Any, content: str, context: Optional[Dict[str, Any]]) -> List[ManipulationResult]:
        results = []
        pattern_analysis = analysis_results['pattern_analysis']
        pattern_matches = pattern_analysis.get('pattern_matches', {})
        if not pattern_matches: return results
        ...
        vulnerability_exploitation_score = self._assess_vulnerability_exploitation(analysis_results, context)
    ...
    def _assess_vulnerability_exploitation(self, analysis_results: Dict[str, Any], context: Optional[Dict[str, Any]]) -> float:
        vulnerability_assessment = analysis_results.get('vulnerability_assessment', {})
        if not vulnerability_assessment: return 0.0
    ...
    def _calculate_protection_priority(self, threat_severity: ThreatSeverity, confidence: float, vulnerability_exploitation: float) -> int:
        severity_scores = {
    ...
    def _audit_detection(self, action: Any, results: List[ManipulationResult], context: Optional[Dict[str, Any]]) -> None:
        audit_entry = {

...
class BaseDetector(ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"Initialized detector '{self.name}' v{self.version} with status {self.status.name}"
        )
            ...
            if principle == EthicalPrinciple.NON_MALEFICENCE:
                ...
                if harm_prevented == 0 and len(violations) > 0:
                    ...
                    recommendations.append(
                        "Consider reviewing low-severity violations for potential harm"
                    )

            elif principle == EthicalPrinciple.JUSTICE:
                # Check for bias in detection
                if hasattr(action, "user_demographics"):
                    bias_indicators = self._assess_bias(
                        action, len(violations) > 0, action.user_demographics
                    )
        ...
        async with self._execution_context() as execution_id:
            try:
                ...
                result = await asyncio.wait_for(
                    self.detect_violations(action), timeout=self.timeout
                )
        ...
        for pii_type, pattern in self.pii_patterns.items():
            ...
            if matches:
                ...
                severity = (
                    ...
                    if pii_type in ["ssn", "credit_card", "passport"]

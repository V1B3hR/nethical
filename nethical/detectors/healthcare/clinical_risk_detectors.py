...
OVERRIDE_PATTERNS = [
    re.compile(
        r"(ignore|bypass|override)\s+(safety|policy|instructions)", re.IGNORECASE
    ),
...
EMERGENCY_PATTERNS = {
    ...
    "overdose": re.compile(
        r"(overdose|took too many|excess (pills|medication))", re.IGNORECASE
    ),
    "acute_distress": re.compile(
        r"(chest pain|shortness of breath|stroke symptoms)", re.IGNORECASE
    ),
}
...
class ClinicalSignals:
    ...
    def condition_mentioned(self) -> bool:
        return bool(
            re.search(
                r"(pregnan|kidney failure|liver disease|hypertension|asthma|diabetes)",
                self.text,
                re.IGNORECASE,
            )
        )
...
def extract_clinical_signals(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "safety": {"emergency_intent": sig.emergency_intent()},
    }

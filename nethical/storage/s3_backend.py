...
class S3Backend:
    ...
    def _initialize_client(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"S3 backend connected to " f"{self.config.endpoint_url or 'AWS S3'}"
            )
            ...
            if error_code == "404":
                try:
                    ...
                    if (
                        ...
                        and not self.config.endpoint_url
                    ):
    ...
    def download_model(self, model_name: str, version: str) -> Optional[bytes]:
        """
    ...
    def download_artifact(
        self, artifact_type: str, artifact_id: str, filename: Optional[str] = None
    ) -> Optional[bytes]:
    ...
    def list_artifacts(
        self, artifact_type: Optional[str] = None, max_results: int = 1000
    ) -> List[ObjectMetadata]:
    ...
    def list_audit_logs(
        ...
        if end_date:
            objects = [obj for obj in objects if obj.last_modified <= end_date]
    ...
    def list_backups(
        self, backup_type: Optional[str] = None, max_results: int = 100
    ) -> List[ObjectMetadata]:
    ...
    def _upload_object(
        ...
        try:
            ...
            self._client.upload_fileobj(
                Fileobj=body, Bucket=bucket, Key=key, ExtraArgs=extra_args
            )
    ...
    def _list_objects(
        ...
        try:
            ...
            page_iterator = paginator.paginate(
                Bucket=bucket, Prefix=prefix, PaginationConfig={"MaxItems": max_results}
            )
    ...
    def stream_download(
        self, bucket: str, key: str, chunk_size: int = 1024 * 1024  # 1MB chunks

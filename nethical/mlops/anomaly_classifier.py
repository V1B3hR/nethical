...
class AnomalyMLClassifier:
    ...
    def _extract_ngrams(self, sequence: List[str]) -> List[Tuple]:
        ...
        return [
            tuple(sequence[i : i + self.n]) for i in range(len(sequence) - self.n + 1)
        ]
    ...
    def train(self, train_data: List[Dict[str, Any]]) -> None:
        ...
        if normal_entropies:
            ...
            variance = sum((e - mean_entropy) ** 2 for e in normal_entropies) / len(
                normal_entropies
            )
    ...
    def predict(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"label": label, "score": final_score, "confidence": confidence}
    ...
    def load(cls, filepath: str) -> "AnomalyMLClassifier":
        ...
        classifier.normal_ngrams = {
            ast.literal_eval(k): v for k, v in normal_ngrams_str.items()
        }
        classifier.anomalous_ngrams = {
            ast.literal_eval(k): v for k, v in anomal_ngrams_str.items()
        }

        classifier.action_frequencies = model_data.get("action_frequencies", {})
        classifier.normal_entropy_stats = model_data.get(
            "normal_entropy_stats", {"mean": 0.0, "std": 1.0}
        )
...
if _TORCH_AVAILABLE:
    ...
    def build_tokenizer(sequences):
        ...
        tok = {
            a: i + 1 for i, a in enumerate(sorted(actions))
    ...
    def extract_features(sequence):
        ...
        features["shannon_entropy"] = scipy_entropy(
            [act_counts[a] / len(sequence) for a in act_counts]
        )
        ...
        trans_freq = (
            [v / len(transitions) for v in trans_counts.values()]
            if transitions
            else [1.0]
        )
        ...
        rare_events = [
            action for action, cnt in act_counts.items() if cnt < (0.05 * len(sequence))
        ]
        ...
        features["avg_action_runlength"] = np.mean(
            [len(list(group)) for _, group in groupby(sequence)]
        )
    ...
    class TransformerAnomalyClassifier(nn.Module):
        ...
        def __init__(
            self, vocab_size, feature_dim, d_model=64, nhead=4, num_layers=2, max_len=48
        ):
            super().__init__()

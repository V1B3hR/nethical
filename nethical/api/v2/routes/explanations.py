...
def _generate_explanation(
    ...
    if risk_score > 0:
        risk_level = (
            ...
            if risk_score < 0.3
                ...
                "moderate"
                if risk_score < 0.6
                else "high" if risk_score < 0.8 else "critical"
            )
        )
    ...
    return ExplanationResponseV2(
        ...
        consequences=consequences_map.get(
            decision, "Standard governance evaluation applied."
        ),
        data_categories=[
            "Action content",
            "Agent identifier",
...
async def get_explanation(
    ...
    include_factors: bool = Query(
        default=True, description="Include contributing factors"
    ),
    include_appeal_info: bool = Query(
        default=True, description="Include appeal information"
    ),

...
class GDPRComplianceValidator:
    ...
    def validate_article_5(
        ...
        if processing_activities.get("purpose_documented"):
            ...
            recommendations.append(
                "Document specific, explicit purposes for data processing"
            )
    ...
    def _generate_human_readable_explanation(
        ...
        parts = [
            f"The governance system evaluated your action and made a '{decision}' decision."
        ]
        ...
        if violations and isinstance(violations, list):
            ...
            for v in violations[:3]:
                if isinstance(v, dict):
                    parts.append(
                        f"- {v.get('type', 'Unknown')}: {v.get('description', 'N/A')}"
                    )
                else:
                    parts.append(f"- {v}")

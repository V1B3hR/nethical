...
class RedTeamManager:
    ...
    def update_metrics(
        self, engagement_id: str, success_rate: float, detection_rate: float
    ) -> None:
        ...
        return {
            ...
            "vulnerability_summary": {
                "total_vulnerabilities": len(
                    self.test_manager.vulnerability_scanner.vulnerabilities
                ),
                "by_severity": {
                    severity.value: len(
                        self.test_manager.vulnerability_scanner.get_vulnerabilities_by_severity(
                            severity
                        )
                    )
                    for severity in VulnerabilitySeverity
                ...
                "by_status": {
                    status.value: len(
                        self.test_manager.vulnerability_scanner.get_vulnerabilities_by_status(
                            status
                        )
                    )
                ...
                "overdue": len(
                    self.test_manager.vulnerability_scanner.get_overdue_vulnerabilities()
                ),
            },
            "penetration_tests": {
            ...
            "red_team_engagements": {
                ...
                "engagements": [
                    eng.to_dict() for eng in self.red_team_manager.engagements.values()
                ],
            },

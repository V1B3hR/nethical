...
class HybridTLSManager:
    ...
    def perform_hybrid_handshake(
        ...
        combined_key = self._combine_secrets(
            classical_secret,
            quantum_result.shared_secret if quantum_result else None
        )
    ...
    def _combine_secrets(
        ...
        if self.hybrid_mode == HybridMode.HYBRID_CONCATENATE:
            ...
            return bytes(a ^ b for a, b in zip(
                classical_secret[:min_len],
                quantum_secret[:min_len]
            ))
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'hybrid_rate': self.hybrid_success_count / self.handshake_count
            if self.handshake_count > 0 else 0.0,
            'hybrid_mode': self.hybrid_mode.value,
...
class QuantumThreatAnalyzer:
    ...
    def __init__(
        self,
        current_qubit_count: int = 1000,
        error_correction_progress: float = 0.3
    ):
    ...
    def assess_quantum_threat(
        ...
        recommended = self._recommend_algorithms(
            criticality_level, affected_systems
        )
    ...
    def _calculate_agility_score(
        ...
        has_pqc = any(
            alg.lower() in inv.lower()
            for alg in quantum_resistant
...
class PQCMigrationPlanner:
    ...
    def complete_phase(self, phase_number: int) -> bool:
        ...
        next_phase = next(
            (p for p in self.phases if p.phase_number == phase_number + 1),
            None
        )
    ...
    def get_migration_status(self) -> Dict[str, Any]:
        ...
        current_phase = next(
            (p for p in self.phases if p.status == "in_progress"),
            None
        )
...
class QuantumCryptoManager:
    ...
    def __init__(
        ...
        self.dilithium = CRYSTALSDilithium(algorithm=dilithium_algorithm) if enable_dilithium else None
        self.hybrid_tls = HybridTLSManager(pqc_algorithm=kyber_algorithm) if enable_hybrid_tls else None

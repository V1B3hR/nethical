...
class SecretRotationManager:
    ...
    def rotate_secret(
        ...
        log.info(
            f"Secret rotated: type={secret.secret_type.value}, rotation_count={new_secret.rotation_count}"
        )
        return new_secret
...
class SecretManagementSystem:
    ...
    def _init_default_policies(self) -> None:
        ...
        self.rotation_manager.add_policy(
            SecretRotationPolicy(
                secret_type=SecretType.API_KEY,
    ...
    def rotate_secrets(self) -> List[str]:
        ...
        for secret_id, secret in list(self.secrets.items()):
            if self.rotation_manager.should_rotate(secret):
                new_secret = self.rotation_manager.rotate_secret(secret, self.generator)

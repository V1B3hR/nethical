...
from nethical.explainability.transparency_report import (
...
class MerkleRootsRegistry:
    ...
    def register_event(
        ...
        leaf_hash = self.tamper_store.append_event(
...
class QuarterlyTransparencyReportGenerator:
    ...
    def generate_quarterly_report(
        ...
        anchor = self.merkle_registry.anchor_merkle_root(
            anchor_type="file", receipt=f"Quarterly report {report_id}"
        )
    ...
    def _get_quarter_dates(self, quarter: int, year: int) -> tuple:
        ...
        if end_month == 12:
            period_end = datetime(year + 1, 1, 1, tzinfo=timezone.utc) - timedelta(
                seconds=1
            )
        else:
            period_end = datetime(
                year, end_month + 1, 1, tzinfo=timezone.utc
            ) - timedelta(seconds=1)
    ...
    def _generate_compliance_summary(
        ...
        summary = {
            ...
            "total_violations": len(violations),
            "block_rate": (
                sum(1 for d in decisions if d.get("decision") == "BLOCK")
                / len(decisions)
                * 100
                if decisions
                else 0

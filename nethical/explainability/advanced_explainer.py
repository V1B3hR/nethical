...
class AdvancedExplainer:
    ...
    def explain_decision_with_features(
        ...
        sorted_features = sorted(
            feature_importances,
            key=lambda f: abs(f.importance_score),
            reverse=True
        )
    ...
    def _extract_features(self, judgment_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if ethical_tags:
            ...
            features["ethical_score"] = sum(dimensions.values()) / len(dimensions) if dimensions else 0.0
        else:
    ...
    def _describe_feature_contribution(
        ...
        strength = "strongly" if abs(importance) > 0.15 else "moderately" if abs(importance) > 0.05 else "slightly"
    ...
    def _generate_feature_explanation(
        ...
        # Add summary
        total_positive = sum(f.importance_score for f in feature_importances if f.importance_score > 0)
        total_negative = sum(f.importance_score for f in feature_importances if f.importance_score < 0)
        
        parts.append("")
        if abs(total_positive) > abs(total_negative):
            parts.append(f"Overall: Features pushed towards blocking/restricting the action.")
        elif abs(total_negative) > abs(total_positive):
            parts.append(f"Overall: Features supported allowing the action.")
        else:
            parts.append(f"Overall: Features were balanced between allowing and blocking.")

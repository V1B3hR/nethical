...
class TransparencyReportGenerator:
    ...
    def _analyze_decision_breakdown(
        ...
        return {
            key: dict(value) for key, value in breakdown.items()
        }
    ...
    def _extract_key_insights(
        ...
        if len(total_violations) >= 7:
            ...
            previous_week = sum(total_violations[-14:-7]) if len(total_violations) >= 14 else recent_week
    ...
    def _generate_recommendations(
        ...
        ineffective_policies = [
            name for name, score in policy_effectiveness.items()
            if score < 0.3
        ]
        ...
        recommendations.append(
            "Review this report with stakeholders monthly"
        )
    ...
    def to_markdown(self, report: TransparencyReport) -> str:
        ...
        for policy, score in sorted(
            report.policy_effectiveness.items(),
            key=lambda x: x[1],
            reverse=True
        ):
            md += f"| {policy} | {score:.1%} |\n"

...
class AlertRuleManager:
    ...
    def _register_default_rules(self) -> None:
        ...
        self.register_rule(
            AlertRule(
                ...
                condition=lambda metrics: self._check_latency(
                    metrics, "action_latency", 1.0
                ),
        ...
        self.register_rule(
            AlertRule(
                ...
                condition=lambda metrics: self._check_latency(
                    metrics, "action_latency", 5.0
                ),
                threshold=5.0,  # 5 seconds
                duration=30,  # Fire if true for 30 seconds
                labels={"component": "governance"},
                annotations={
                    "summary": "Critical latency detected - immediate action required"
                },
            )
        )
        ...
        self.register_rule(
            AlertRule(
                ...
                labels={"component": "quota_manager"},
                annotations={
                    "summary": "Quota usage above 95% - immediate action required"
                },
            )
        )

    def _check_latency(
        self, metrics: Dict[str, Any], metric_name: str, threshold: float
    ) -> bool:
    ...
    def _check_quota_saturation(
        self, metrics: Dict[str, Any], threshold: float
    ) -> bool:
            ...
            for rule_name, rule in self.rules.items():
                try:
                    ...
                    if condition_met:
                        ...
                        if (
                            time_active >= rule.duration
                            and rule.state != AlertState.ACTIVE
                        ):
                            # Fire alert
    ...
    def clear_history(self, before: Optional[datetime] = None) -> int:
        ...
        with self._lock:
            if before:
                ...
                self.alert_history = [
                    a for a in self.alert_history if a.timestamp >= before
                ]
...
def console_alert_handler(alert: Alert) -> None:
    ...
    print(
        f"{prefix} [{alert.severity.value.upper()}] {alert.rule_name}: {alert.message}"
    )
...
if __name__ == "__main__":
    ...
    test_metrics = {
        "histograms": {
            "action_latency": {"p95": 6.0}  # 6 seconds - exceeds both thresholds
        },
        "gauges": {
            "error_rate": {"default": 0.08},  # 8% error rate
            "quota_usage": {"agent_001": 0.92},  # 92% quota usage
        },
    }

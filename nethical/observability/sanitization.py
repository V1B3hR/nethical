...
class LogSanitizer:
    ...
    def sanitize_dict(
        ...
        for key, value in data.items():
            ...
            if any(
                ...
                for sensitive in default_sensitive + sensitive_keys_lower
            ):
                result[key] = "[REDACTED]"
                ...
                result[key] = [
                    (
                        self.sanitize_dict(item, recursive, sensitive_keys)
                        if isinstance(item, dict)
                        else self.sanitize(item) if isinstance(item, str) else item
                    )
                    for item in value
                ]
            else:
    ...
    def list_patterns(self) -> List[Dict[str, str]]:
        ...
        """
        return [
            {"name": p.name, "description": p.description, "replacement": p.replacement}
...
def get_sanitizer(
    ...
    if _log_sanitizer is None:
        _log_sanitizer = LogSanitizer(
            enabled_patterns=enabled_patterns, custom_patterns=custom_patterns
        )
...
if __name__ == "__main__":
    ...
    test_dict = {
        ...
        "metadata": {"ssn": "123-45-6789", "safe_data": "This is fine"},
    }

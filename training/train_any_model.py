...
# Model classes imported inside registry function to reduce startup errors
...
def resolve_kaggle_credentials(
    username: Optional[str], key: Optional[str]
...
def download_kaggle_datasets(
    ...
    try:
        ...
        logging.warning(
            "Kaggle package not installed. Skipping dataset downloads. Using synthetic or local data."
        )
        return
    resolved_user, resolved_key = resolve_kaggle_credentials(
    ...
    if resolved_user and resolved_key:
        ...
        logging.info(
            "Kaggle credentials not provided/found. Attempting API auth via existing config if any."
        )
    ...
    for dataset in datasets:
        ...
        try:
            kaggle.api.dataset_download_files(
                dataset, path=str(DATA_EXTERNAL_DIR), unzip=True
            )
                    ...
                    if len(parts) >= 2:
                        ...
                        if len(slug_parts) >= 2:
                            ...
                            if owner and dataset_name:
                                datasets.append(f"{owner}/{dataset_name}")
...
def load_data(
    num_samples: int = 10000, model_type: str = "logistic"
) -> List[Dict[str, Any]]:
    csv_files = list(DATA_EXTERNAL_DIR.glob("*.csv"))
    if csv_files:
        logging.info(
            "Found %d CSV(s) in %s. Loading up to %d samples.",
            len(csv_files),
            DATA_EXTERNAL_DIR,
            num_samples,
        )
        try:
            ...
            for csv_file in csv_files:
                try:
                    ...
                    if not label_col:
                        logging.debug(
                            "No label column found in %s; skipping.", csv_file.name
                        )
                        continue
                    feat_cols = _discover_feature_columns(df)
                    if not feat_cols:
                        logging.debug(
                            "No usable numeric features found in %s; skipping.",
                            csv_file.name,
                        )
                        continue
            ...
            if data:
                return data
            logging.warning(
                "No valid samples found in Kaggle data. Falling back to synthetic."
            )
...
def _generate_synthetic(num_samples: int, model_type: str) -> List[Dict[str, Any]]:
    ...
    if model_type == "anomaly":
        ...
        for _ in range(normal_count):
            data.append(
                {
                    "features": {"sequence": random.choice(normal_patterns).copy()},
                    "label": 0,
                }
            )
        for _ in range(anomalous_count):
            data.append(
                {
                    "features": {"sequence": random.choice(anomalous_patterns).copy()},
                    "label": 1,
                }
            )
        random.shuffle(data)
...
def preprocess_for_heuristic(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for sample in data:
        ...
        if available_preferred:
            ...
            sample["features"] = {
                k: float(v) for k, v in feats.items() if isinstance(v, (int, float))
            }
    return data
...
def preprocess_for_logistic(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if not keys:
        return data
    arr = np.array(
        [[float(sample["features"].get(k, 0.0)) for k in keys] for sample in data],
        dtype=float,
    )
    ...
    for i, sample in enumerate(data):
        sample["features"] = {
            k: float((arr[i, j] - mins[j]) / denom[j]) for j, k in enumerate(keys)
        }
    return data
...
def preprocess_for_correlation(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for pos, idx in enumerate(valid_indices):
        normalized = {
            name: float((arr[pos, j] - mins[j]) / denom[j])
...
def get_model_class(model_type: str):
    ...
    if model_type not in registry:
        raise ValueError(
            f"Unknown model_type: {model_type}. Supported: {list(registry.keys())}"
        )
...
def split_data(
    ...
    logging.info(
        "Train samples: %d | Validation samples: %d", len(train_data), len(val_data)
    )
    return train_data, val_data


def _compute_confusion(
    preds: Sequence[int], labels: Sequence[int]
...
def _ece(probs: Sequence[float], labels: Sequence[int], n_bins: int = 10) -> float:
    ...
    for b in range(n_bins):
        ...
        acc = float(
            np.mean(
                [
                    int((bin_probs[i] >= 0.5) == bool(bin_labels[i]))
...
def _roc_auc_safe(labels: Sequence[int], probs: Sequence[float]) -> float:
    # Try sklearn if available; else return NaN
    try:
        from sklearn.metrics import roc_auc_score  # type: ignore

        if len(labels) > 0 and len(probs) == len(labels) and len(set(labels)) > 1:
...
def check_promotion_gate(
    ...
    apply_ece = {"strict": True, "probs_only": bool(has_probs), "disable": False}.get(
        ece_policy, True
    )
    ...
    logging.info(
        "Promotion Gate Policy: %s | Conditions: %s",
        ece_policy,
        " AND ".join(conditions),
    )
    ...
    logging.info(
        "Gate Values: Accuracy=%.3f (%s) | ECE=%.3f (%s) | Probabilities=%s",
        acc,
...
async def validate_with_governance(
    governance, content, action_type, action_id, agent_id="training_pipeline"
):
    action = AgentAction(
        action_id=action_id,
        agent_id=agent_id,
            ...
            try:
                judgment = new_loop.run_until_complete(
                    validate_with_governance(
                        governance, content, action_type, action_id, agent_id
                    )
                )
            finally:
                new_loop.close()
...
def save_model_and_metrics(
    ...
    if hasattr(model, "save") and callable(getattr(model, "save")):
        ...
        with open(model_path, "w") as f:
            json.dump(
                {
                    "warning": "model.save missing; stored stub only",
                    "model_type": model_type,
                },
                f,
            )
        logging.warning(
            "[%s] Model class lacks save(); wrote stub JSON at %s",
            model_type,
            model_path,
        )
...
def train_single_model(
    ...
    try:
        timestamp = batch_timestamp or datetime.now(timezone.utc).strftime(
            "%Y%m%d_%H%M%S"
        )
        ...
        if args.include_adversarial:
            if ADVERSARIAL_AVAILABLE:
                logging.info(
                    "[%s] Adversarial Training Enabled: Generating %d threats per type...",
                    model_type,
                    args.adversarial_count,
                )
                ...
                logging.info(
                    "[%s] Added %d adversarial samples. Total dataset size: %d",
                    model_type,
                    len(adv_data),
                    len(raw_data),
                )
                ...
                logging.warning(
                    "[%s] Adversarial generator requested but module not found.",
                    model_type,
                )
        ...
        if governance:
            logging.info(
                "[%s] Running governance validation on training data samples...",
                model_type,
            )
            ...
            for i, sample in enumerate(raw_data[:sample_size]):
                try:
                    ...
                    logging.warning(
                        "[%s] Error validating sample %d: %s", model_type, i, e
                    )
            result["data_violations"] = len(governance_violations)
            if governance_violations:
                logging.warning(
                    "[%s] Governance found %d problematic data samples",
                    model_type,
                    len(governance_violations),
                )
                if merkle_anchor:
                    merkle_anchor.add_event(
                        {
                ...
                if args.gov_fail_on_violations:
                    result["error"] = (
                        "Governance violations in data (fail-fast enabled)"
                    )
                    return result
            else:
                logging.info(
                    "[%s] Governance validation passed for %d data samples",
                    model_type,
                    sample_size,
                )
        ...
        if len(train_data) == 0 or len(val_data) == 0:
            result["error"] = (
                "Empty train or validation split; check split strategy or dataset size."
            )
        ...
        logging.info(
            "[%s] Training model for %d epoch(s), batch size %d...",
            model_type,
            args.epochs,
            args.batch_size,
        )
        training_start_time = datetime.now(timezone.utc)
        # Note: baseline models may ignore epochs/batch_size
        ...
        metrics = compute_metrics(
            preds, labels, probs=probs_all, ece_bins=args.ece_bins
        )
        ...
        if governance:
            logging.info(
                "[%s] Running governance validation on model predictions...", model_type
            )
            ...
            for i in range(sample_size):
                try:
                    ...
                    logging.warning(
                        "[%s] Error validating prediction %d: %s", model_type, i, e
                    )
            result["prediction_violations"] = len(prediction_violations)
            if prediction_violations:
                logging.warning(
                    "[%s] Governance found %d problematic predictions",
                    model_type,
                    len(prediction_violations),
                )
                if merkle_anchor:
                    merkle_anchor.add_event(
                        {
                ...
                if args.gov_fail_on_violations:
                    result["error"] = (
                        "Governance violations in predictions (fail-fast enabled)"
                    )
                    return result
            else:
                logging.info(
                    "[%s] Governance validation passed for %d predictions",
                    model_type,
                    sample_size,
                )
        ...
        if drift_reporter:
            ...
            try:
                ...
                logging.warning(
                    "[%s] Failed to track action in drift reporter: %s", model_type, e
                )

        has_probs = probs_all is not None
        logging.info(
            "[%s] Probabilities available for ECE/Brier: %s",
            model_type,
            "yes" if has_probs else "no",
        )
            ...
            try:
                if metrics.get("ece", 0.0) > args.promotion_max_ece:
                    drift_reporter.track_violation(
                        ...
                        severity=(
                            "high"
                            if metrics["ece"] > (args.promotion_max_ece * 2)
                            else "medium"
                        ),
                    )
                if metrics.get("accuracy", 1.0) < args.promotion_min_accuracy:
                    drift_reporter.track_violation(
                        ...
                        severity=(
                            "high"
                            if metrics["accuracy"]
                            < max(0.7, args.promotion_min_accuracy - 0.15)
                            else "medium"
                        ),
                    )
            except Exception as e:
                logging.warning(
                    "[%s] Failed to record drift violations: %s", model_type, e
                )
        ...
        if governance:
            ...
            logging.info(
                "  Data samples validated: %d",
                min(args.gov_data_samples, len(raw_data)),
            )
            logging.info("  Data violations found: %d", len(governance_violations))
            logging.info(
                "  Predictions validated: %d", min(args.gov_pred_samples, len(val_data))
            )
            logging.info(
                "  Prediction violations found: %d", len(prediction_violations)
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of epochs (may be unused for baseline models)",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=32,
        help="Batch size (may be unused for baseline models)",
    )
    parser.add_argument(
        "--num-samples", type=int, default=10000, help="Number of samples to use"
    )
    ...
    parser.add_argument(
        ...
        default=1,
        choices=[0, 1, 2],
        help="Logging verbosity: 0=warn, 1=info, 2=debug",
    )

    # Data and split
    parser.add_argument(
        "--train-ratio", type=float, default=0.8, help="Train split ratio (0,1)"
    )
    parser.add_argument(
        "--split-strategy",
        type=str,
    ...
    parser.add_argument(
        "--no-download",
        action="store_true",
        help="Skip downloading datasets from Kaggle",
    )

    # Kaggle
    parser.add_argument(
        "--kaggle-username", type=str, default=None, help="Kaggle username (optional)"
    )
    parser.add_argument(
        "--kaggle-key", type=str, default=None, help="Kaggle API key (optional)"
    )
    parser.add_argument(
        "--overwrite-kaggle-json",
        action="store_true",
        help="Overwrite existing ~/.kaggle/kaggle.json",
    )
    parser.add_argument(
        "--kaggle-dataset",
        action="append",
        default=[],
        help="Kaggle dataset slug to download (can be repeated)",
    )

    # Audit
    parser.add_argument(
        "--enable-audit", action="store_true", help="Enable Merkle audit logging"
    )
    parser.add_argument(
        "--audit-path",
        type=str,
        default="training_audit_logs",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable ethical drift tracking",
    )
    parser.add_argument(
        "--drift-report-dir",
        type=str,
        default="training_drift_reports",
        help="Directory for drift reports",
    )
    parser.add_argument(
        "--cohort-id",
        type=str,
        default=None,
        help="Cohort identifier for drift tracking",
    )
    ...
    parser.add_argument(
        "--enable-governance",
        action="store_true",
        help="Enable governance validation during training",
    )
    parser.add_argument(
        "--gov-data-samples",
        type=int,
        default=100,
        help="Number of training samples to validate with governance",
    )
    parser.add_argument(
        "--gov-pred-samples",
        type=int,
        default=50,
        help="Number of prediction samples to validate with governance",
    )
    parser.add_argument(
        "--gov-fail-on-violations",
        action="store_true",
        help="Abort training if governance finds any violations",
    )
    ...
    parser.add_argument(
        "--include-adversarial",
        action="store_true",
        help="Mix in synthetic adversarial threats (Hard Negatives)",
    )
    parser.add_argument(
        "--adversarial-count",
        type=int,
        default=200,
        help="Number of adversarial samples to generate per type",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.08,
        help="Max ECE to pass promotion gate",
    )
    parser.add_argument(
        "--promotion-min-accuracy",
        type=float,
        default=0.85,
        help="Min accuracy to pass promotion gate",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of bins to compute Expected Calibration Error (ECE)",
    )
    parser.add_argument(
        "--ece-policy",
        type=str,
        default="strict",
        choices=["strict", "probs_only", "disable"],
        help="How to apply ECE in the promotion gate",
    )
...
def main():
    ...
    if args.model_type.lower() == TRAIN_ALL_KEYWORD:
        ...
        if args.model_type not in ALL_MODEL_TYPES:
            logging.error(
                "Unknown model type: %s. Available types: %s, or '%s'",
                args.model_type,
                ", ".join(ALL_MODEL_TYPES),
                TRAIN_ALL_KEYWORD,
            )
            sys.exit(1)
        ...
        if MERKLE_AVAILABLE:
            try:
                merkle_anchor = MerkleAnchor(
                    storage_path=args.audit_path, chunk_size=100
                )
                logging.info(
                    "Merkle audit logging enabled. Logs stored in: %s", args.audit_path
                )
                merkle_anchor.add_event(
                    {
    ...
    if args.enable_drift_tracking:
        if DRIFT_REPORTER_AVAILABLE:
            try:
                ...
                logging.info(
                    "Ethical drift tracking enabled. Reports stored in: %s",
            ...
            logging.warning(
                "Drift tracking requested but EthicalDriftReporter not available"
            )
    ...
    if args.enable_governance:
        if GOVERNANCE_AVAILABLE:
            ...
            logging.warning(
                "Governance validation requested but EnhancedSafetyGovernance not available"
            )
    ...
    for i, model_type in enumerate(model_types_to_train, 1):
        logging.info("=" * 70)
        logging.info(
            "Training model %d/%d: %s", i, len(model_types_to_train), model_type
        )
        logging.info("=" * 70)
        ...
        if result["success"]:
            logging.info(
                "[%s] Training completed successfully. Promoted: %s",
                model_type,
                result["promoted"],
            )
        else:
    ...
    if drift_reporter:
        try:
            ...
            logging.info(
                "Drift detected: %s", drift_report.drift_metrics.get("has_drift", False)
            )
            ...
            drift_report_path = (
                Path(args.drift_report_dir) / f"{drift_report.report_id}.json"
            )
    ...
    if merkle_anchor:
        try:
            ...
            if (
                getattr(merkle_anchor, "current_chunk", None)
                and merkle_anchor.current_chunk.event_count > 0
            ):
                merkle_root = merkle_anchor.finalize_chunk()
                logging.info(
                    "Training audit trail finalized. Merkle root: %s", merkle_root
                )
                ...
                if governance:
                    ...
                    if (
                        isinstance(governance_metrics, dict)
                        and "metrics" in governance_metrics
                    ):
                    ...
                    gov_summary["data_violations"] = sum(
                        r.get("data_violations", 0) for r in all_results
                    )
                    gov_summary["prediction_violations"] = sum(
                        r.get("prediction_violations", 0) for r in all_results
                    )
    ...
    if governance:
        ...
        try:
            ...
            if (
                "metrics" in gov_metrics
                and "total_actions_evaluated" in gov_metrics["metrics"]
            ):
                logging.info(
                    "  Total governance actions: %s",
                    gov_metrics["metrics"]["total_actions_evaluated"],
                )
                logging.info(
                    "  Total violations detected: %s",
                    gov_metrics["metrics"]["total_violations_detected"],
                )
        except Exception:
            pass

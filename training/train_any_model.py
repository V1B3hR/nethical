...
def download_kaggle_datasets(
    ...
    try:
        ...
        logging.warning("Kaggle package not installed. Skipping dataset downloads. Using synthetic or local data.")
        return
...
async def validate_with_governance(governance, content, action_type, action_id, agent_id="training_pipeline"):
    action = AgentAction(action_id=action_id, agent_id=agent_id, action_type=action_type, content=str(content))
...
def save_model_and_metrics(model: Any, metrics: Dict[str, float], model_type: str, promoted: bool = False, base_dir: str = "models") -> Tuple[str, str]:
    ...
    if hasattr(model, "save") and callable(getattr(model, "save")):
        ...
        with open(model_path, "w") as f:
            json.dump({"warning": "model.save missing; stored stub only", "model_type": model_type}, f)
        logging.warning("[%s] Model class lacks save(); wrote stub JSON at %s", model_type, model_path)
    ...
    try:
        ...
        if args.include_adversarial:
            if ADVERSARIAL_AVAILABLE:
                logging.info("[%s] Adversarial Training Enabled: Generating %d threats per type...", model_type, args.adversarial_count)
                ...
                logging.info("[%s] Added %d adversarial samples. Total dataset size: %d", model_type, len(adv_data), len(raw_data))
            else:
        ...
        if governance:
            ...
            if governance_violations:
                logging.warning("[%s] Governance found %d problematic data samples", model_type, len(governance_violations))
                if merkle_anchor:
                    merkle_anchor.add_event({
        ...
        if len(train_data) == 0 or len(val_data) == 0:
            result['error'] = "Empty train or validation split; check split strategy or dataset size."
        ...
        logging.info("[%s] Training model for %d epoch(s), batch size %d...", model_type, args.epochs, args.batch_size)
        ...
        if governance:
            ...
            if prediction_violations:
                logging.warning("[%s] Governance found %d problematic predictions", model_type, len(prediction_violations))
                if merkle_anchor:
                    merkle_anchor.add_event({
        ...
        model_path, metrics_path = save_model_and_metrics(model=model, metrics=metrics, model_type=model_type, promoted=promoted, base_dir=args.models_dir)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model-type", type=str, required=True, help=f"Model type to train. Options: {all_types_str}, or '{TRAIN_ALL_KEYWORD}' to train all model types")
...
def main():
    ...
    if args.model_type.lower() == TRAIN_ALL_KEYWORD:
        ...
        if args.model_type not in ALL_MODEL_TYPES:
            logging.error("Unknown model type: %s. Available types: %s, or '%s'", args.model_type, ", ".join(ALL_MODEL_TYPES), TRAIN_ALL_KEYWORD)
            sys.exit(1)
    ...
    for i, model_type in enumerate(model_types_to_train, 1):
        ...
        if result['success']:
            logging.info("[%s] Training completed successfully. Promoted: %s", model_type, result['promoted'])
        else:
    ...
    if drift_reporter:
        try:
            ...
            drift_report = drift_reporter.generate_report(start_time=training_start_time, end_time=training_end_time)
